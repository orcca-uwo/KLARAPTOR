...
#include "../../src/trace_analysis/mem_inst_trace.h"
using namespace std;
...
def cuda_to_ptx(kernel_name_file_list, arch, opt_level):
	...
	for i in range(n):
		...
		cmd="../../src/trace_analysis/cu_to_ptx.sh "+file+ " "+arch+" "+str(opt_level);
		print_msg("["+cmd+"]");	
		os.system(cmd);
...
def main():
	...
	if argc<1:
		print("args: [1]:src_path")
		exit(-1);
	else:
		src_path=sys.argv[1]	
		print_msg("[@extract_kernel_name_list.py][src=%s]..."%(src_path));
		parse(src_path);
