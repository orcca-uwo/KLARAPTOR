	...
	integer *lda)
{
    ...
    double pow_dd(doublereal *, doublereal *), pow_di(doublereal *, integer *)
	    , log(doublereal), exp(doublereal), sqrt(doublereal);
    ...
    static doublereal safmin, sv1, sv2;
...
L140:
	...
	if (klen > 1) {
		...
		i__2 = i__ - 1;
		a_ref(*nz1 + i__, *nz1 + i__) = pow_di(&alpha, &i__2);
/* L150: */
	    }
	}
	goto L220;
...
L160:
	...
	if (klen > 1) {
	    ...
	    i__1 = klen;
	    for (i__ = 2; i__ <= i__1; ++i__) {
		a_ref(*nz1 + i__, *nz1 + i__) = (doublereal) (klen - i__) * 
			alpha + *rcond;
/* L170: */
	    }
	}
...
L180:
	...
	i__1 = kend;
	for (jd = kbeg; jd <= i__1; ++jd) {
	    a_ref(jd, jd) = exp(alpha * dlaran_(&iseed[1]));
/* L190: */
	}
	goto L220;
...
L200:
	i__1 = kend;
	for (jd = kbeg; jd <= i__1; ++jd) {
	    a_ref(jd, jd) = dlarnd_(idist, &iseed[1]);
/* L210: */
	}
...
/*     Fill in upper triangle (except for 2x2 blocks) */
	...
	if (*isign != 2 || *itype == 2 || *itype == 3) {
	    ...
	    for (jr = 1; jr <= i__1; ++jr) {
		if (a_ref(jr + 1, jr) == 0.) {
		    a_ref(jr, jr + 1) = *triang * dlarnd_(idist, &iseed[1]);
		}
/* L300: */
	    }
	}
	...
	for (jc = 2; jc <= i__1; ++jc) {
	    i__2 = jc - ioff;
	    for (jr = 1; jr <= i__2; ++jr) {
		a_ref(jr, jc) = *triang * dlarnd_(idist, &iseed[1]);
/* L310: */
	    }
/* L320: */
	}
