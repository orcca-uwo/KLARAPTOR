...
/*              Copy VT to WORK(IL), multiply right singular vectors of L   
		...
		latime_1.ops += dopbl3_("ZGEMM ", m, n, m);
...
/*              Generate Q in VT   
		...
		nb = ilaenv_(&c__1, "ZUNGLQ", " ", n, n, m, &c_n1, (ftnlen)6, 
			(ftnlen)1);
		latime_1.ops += dopla_("ZUNGLQ", n, n, m, &c__0, &nb);
		i__1 = *lwork - nwork + 1;
...
/*              Produce L in A, zeroing out above it */
		...
		i__1 = *m - 1;
		i__2 = *m - 1;
		zlaset_("U", &i__1, &i__2, &c_b1, &c_b1, &a_ref(1, 2), lda);
		ie = 1;
		itauq = itau;
		itaup = itauq + *m;
		nwork = itaup + *m;
...
/*              Generate P**H in A   
		...
		nb = ilaenv_(&c__1, "ZUNGBR", "P", m, n, m, &c_n1, (ftnlen)6, 
			(ftnlen)1);
		latime_1.ops += dopla2_("ZUNGBR", "P", m, n, m, &c__0, &nb);
		i__1 = *lwork - nwork + 1;
...
/*              Multiply Q in U by real matrix RWORK(IRVT)   
		...
		latime_1.ops += (doublereal) ((*m << 2) * *m * *m);
...
/* Computing MIN */
		    i__3 = *n - i__ + 1;
		    blk = min(i__3,chunk);
		    latime_1.ops += (doublereal) ((*m << 2) * blk * *m);
		    zlarcm_(m, &blk, &rwork[irvt], m, &a_ref(1, i__), lda, &
			    work[ivt], &ldwkvt, &rwork[nrwork]);
		    zlacpy_("F", m, &blk, &work[ivt], &ldwkvt, &a_ref(1, i__),
			     lda);
/* L50: */
		}
	    } else if (wntqs) {
...
/*              Multiply real matrix RWORK(IRVT) by P**H in VT,   
		...
		nrwork = iru;
		latime_1.ops += (doublereal) ((*m << 2) * *n * *m);
...
/*              Copy A to VT, generate P**H   
		...
		zlacpy_("U", m, n, &a[a_offset], lda, &vt[vt_offset], ldvt);
		nb = ilaenv_(&c__1, "ZUNGBR", "P", n, n, m, &c_n1, (ftnlen)6, 
			(ftnlen)1);
		latime_1.ops += dopla2_("ZUNGBR", "P", n, n, m, &c__0, &nb);
		i__2 = *lwork - nwork + 1;
...
/*              Copy real matrix RWORK(IRVT) to complex matrix WORK(IVT)   
		    ...
		    zlacp2_("F", m, m, &rwork[irvt], m, &work[ivt], &ldwkvt);
		    nb = ilaenv_(&c__1, "ZUNMBR", "PRC", m, n, m, &c_n1, (
			    ftnlen)6, (ftnlen)3);
		    latime_1.ops += dopla2_("ZUNMBR", "PRC", m, n, m, &c__0, &
			    nb);
		    i__2 = *lwork - nwork + 1;
...
/*                 Generate P**H in A   
		    ...
		    nb = ilaenv_(&c__1, "ZUNGBR", "P", m, n, m, &c_n1, (
			    ftnlen)6, (ftnlen)1);
		    latime_1.ops += dopla2_("ZUNGBR", "P", m, n, m, &c__0, &
			    nb);
		    i__2 = *lwork - nwork + 1;
...
/* Computing MIN */
			...
			zlarcm_(m, &blk, &rwork[irvt], m, &a_ref(1, i__), lda,
				 &work[ivt], &ldwkvt, &rwork[nrwork]);
			zlacpy_("F", m, &blk, &work[ivt], &ldwkvt, &a_ref(1, 
				i__), lda);
/* L60: */
		    }
		}
...
/*              Copy real matrix RWORK(IRVT) to complex matrix VT   
		...
		zlacp2_("F", m, m, &rwork[irvt], m, &vt[vt_offset], ldvt);
		nb = ilaenv_(&c__1, "ZUNMBR", "PRC", m, n, m, &c_n1, (ftnlen)
			6, (ftnlen)3);
		latime_1.ops += dopla2_("ZUNMBR", "PRC", m, n, m, &c__0, &nb);
		i__1 = *lwork - nwork + 1;
		zunmbr_("P", "R", "C", m, n, m, &a[a_offset], lda, &work[
			itaup], &vt[vt_offset], ldvt, &work[nwork], &i__1, &
			ierr);
...
/*              Set the right corner of VT to identity matrix */
		...
		i__1 = *n - *m;
		i__2 = *n - *m;
		zlaset_("F", &i__1, &i__2, &c_b1, &c_b2, &vt_ref(*m + 1, *m + 
			1), ldvt);
...
/*              Copy real matrix RWORK(IRVT) to complex matrix VT   
		...
		zlacp2_("F", m, m, &rwork[irvt], m, &vt[vt_offset], ldvt);
		nb = ilaenv_(&c__1, "ZUNMBR", "PRC", n, n, m, &c_n1, (ftnlen)
			6, (ftnlen)3);
		latime_1.ops += dopla2_("ZUNMBR", "PRC", n, n, m, &c__0, &nb);
		i__1 = *lwork - nwork + 1;
...
/*     Undo scaling if necessary */
    ...
    if (iscl == 1) {
	if (anrm > bignum) {
	    latime_1.ops += (doublereal) (minmn * 6);
	    dlascl_("G", &c__0, &c__0, &bignum, &anrm, &minmn, &c__1, &s[1], &
		    minmn, &ierr);
	}
	if (*info != 0 && anrm > bignum) {
	    latime_1.ops += (doublereal) ((minmn - 1) * 6);
	    i__1 = minmn - 1;
