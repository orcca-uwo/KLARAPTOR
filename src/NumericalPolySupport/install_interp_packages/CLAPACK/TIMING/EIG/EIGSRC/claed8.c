...
/*     If the rank-1 modifier is small enough, no more needs to be done -   
	...
	*k = 0;
	...
	for (j = 1; j <= i__1; ++j) {
	    perm[j] = indxq[indx[j]];
	    ccopy_(qsiz, &q_ref(1, perm[j]), &c__1, &q2_ref(1, j), &c__1);
/* L50: */
	}
	clacpy_("A", qsiz, n, &q2_ref(1, 1), ldq2, &q_ref(1, 1), ldq);
	return 0;
    }
...
/*           Record the appropriate Givens rotation */
	    ...
	    csrot_(qsiz, &q_ref(1, indxq[indx[jlam]]), &c__1, &q_ref(1, indxq[
		    indx[j]]), &c__1, &c__, &s);
	    latime_1.ops += 10;
...
/*     The deflated eigenvalues and their corresponding vectors go back   
	...
	i__1 = *n - *k;
	clacpy_("A", qsiz, &i__1, &q2_ref(1, *k + 1), ldq2, &q_ref(1, *k + 1),
		 ldq);
    }
