...
/*     If matrix lower bidiagonal, rotate to be upper bidiagonal   
    ...
    if (lower) {
	latime_1.ops += (real) (*n - 1) * ((*nru + *ncc) * 6 + 8);
	i__1 = *n - 1;
...
/*        2 by 2 block, handle separately */
	...
	latime_1.ops = latime_1.ops + 37 + (*ncvt + *nru + *ncc) * 6;
...
/*     If SHIFT = 0, do simplified QR iteration */
    ...
    if (shift == 0.f) {
	latime_1.ops = latime_1.ops + 2 + (real) (m - ll) * ((*ncvt + *nru + *
		ncc) * 6 + 20);
	if (idir == 1) {
...
/*        Use nonzero shift */
	...
	latime_1.ops = latime_1.ops + 2 + (m - ll) * ((*ncvt + *nru + *ncc) * 
		6 + 32);
	if (idir == 1) {
