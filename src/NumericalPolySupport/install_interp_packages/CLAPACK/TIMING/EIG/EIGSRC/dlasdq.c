...
/*        Update singular vectors if desired. */
	...
	if (*ncvt > 0) {
	    latime_1.ops += (doublereal) ((np1 - 1) * 6 * *ncvt);
...
/*        Update singular vectors if desired. */
	...
	if (*nru > 0) {
	    if (sqre1 == 0) {
		latime_1.ops += (doublereal) ((*n - 1) * 6 * *nru);
		dlasr_("R", "V", "F", nru, n, &work[1], &work[np1], &u[
			u_offset], ldu);
	    } else {
		latime_1.ops += (doublereal) (*n * 6 * *nru);
	...
	if (*ncc > 0) {
	    if (sqre1 == 0) {
		latime_1.ops += (doublereal) ((*n - 1) * 6 * *ncc);
		dlasr_("L", "V", "F", n, ncc, &work[1], &work[np1], &c__[
			c_offset], ldc);
	    } else {
		latime_1.ops += (doublereal) (*n * 6 * *ncc);
