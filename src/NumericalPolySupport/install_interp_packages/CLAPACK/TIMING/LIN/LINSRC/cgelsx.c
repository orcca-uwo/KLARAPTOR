	 ...
	 integer *rank, complex *work, real *rwork, integer *info)
{
    ...
    static real smaxpr, smlnum, tim1, tim2;
...
/*        Scale matrix norm up to SMLNUM */
	...
	lstime_1.opcnt[gelsx - 1] += (real) (*m * 6 * *n);
...
/*        Scale matrix norm up to SMLNUM */
	...
	lstime_1.opcnt[gelsx - 1] += (real) (*m * 6 * *nrhs);
...
/*     Compute QR factorization with column pivoting of A:   
    ...
    lstime_1.opcnt[geqpf - 1] += sopla_("CGEQPF", m, n, &c__0, &c__0, &c__0);
    tim1 = second_();
...
L10:
	...
	if (smaxpr * *rcond <= sminpr) {
	    lstime_1.opcnt[gelsx - 1] += (real) (*rank * 6);
	    i__1 = *rank;
...
/*     Logically partition R = [ R11 R12 ]   
    ...
    if (*rank < *n) {
	lstime_1.opcnt[tzrqf - 1] += sopla_("CTZRQF", rank, n, &c__0, &c__0, &
		c__0);
	tim1 = second_();
...
/*     Details of Householder rotations stored in WORK(MN+1:2*MN)   
    ...
    lstime_1.opcnt[unm2r - 1] += sopla_("CUNMQR", m, nrhs, &mn, &c__0, &c__0);
    tim1 = second_();
...
/*     B(1:N,1:NRHS) := Y' * B(1:N,1:NRHS) */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    clatzm_("Left", &i__2, nrhs, &a_ref(i__, *rank + 1), lda, &q__1, &
...
/*     Undo scaling */
    ...
    if (iascl == 1) {
	lstime_1.opcnt[gelsx - 1] += (real) ((*n * *nrhs + *rank * *rank) * 6)
		;
    ...
    if (ibscl == 1) {
	lstime_1.opcnt[gelsx - 1] += (real) (*n * 6 * *nrhs);
