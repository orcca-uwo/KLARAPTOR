	...
	iwork, integer *info)
{
    ...
    /* Local variables */
    static integer nlvl, sqre, i__, j, inode, ndiml;
    extern /* Subroutine */ int sgemm_(char *, char *, integer *, integer *, 
	    integer *, real *, real *, integer *, real *, integer *, real *, 
	    real *, integer *);
    static integer ndimr, i1;
...
/*        IC : center row of each node   
	...
	sgemm_("T", "N", &nl, nrhs, &nl, &c_b9, &u_ref(nlf, 1), ldu, &b_ref(
		nlf, 1), ldb, &c_b10, &bx_ref(nlf, 1), ldbx);
	sgemm_("T", "N", &nr, nrhs, &nr, &c_b9, &u_ref(nrf, 1), ldu, &b_ref(
...
/*     Next copy the rows of B that correspond to unchanged rows   
	...
	scopy_(nrhs, &b_ref(ic, 1), ldb, &bx_ref(ic, 1), ldbx);
/* L20: */
    }
...
/*        find the first node LF and last node LL on   
	...
	for (i__ = lf; i__ <= i__1; ++i__) {
	    ...
	    nrf = ic + 1;
	    --j;
	    slals0_(icompq, &nl, &nr, &sqre, nrhs, &bx_ref(nlf, 1), ldbx, &
...
/*        Find the first node LF and last node LL on   
	...
	for (i__ = ll; i__ >= i__2; --i__) {
	    ...
	    ++j;
	    slals0_(icompq, &nl, &nr, &sqre, nrhs, &b_ref(nlf, 1), ldb, &
