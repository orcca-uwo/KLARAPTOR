...
/*        Bidiagonalize L in WORK(IL)   
	    ...
	    nb = ilaenv_(&c__1, "SGEBRD", " ", m, m, &c_n1, &c_n1, (ftnlen)6, 
		    (ftnlen)1);
	    lstime_1.opcnt[gebrd - 1] += sopla_("SGEBRD", m, m, &c__0, &c__0, 
		    &nb);
	    t1 = second_();
	    i__2 = *lwork - iwork + 1;
...
/*        Multiply B by transpose of left bidiagonalizing vectors of L   
	    ...
	    nb = ilaenv_(&c__1, "SORMBR", "QLT", m, nrhs, m, &c_n1, (ftnlen)6,
		     (ftnlen)3);
	    lstime_1.opcnt[ormbr - 1] += sopla2_("SORMBR", "QLT", m, nrhs, m, 
		    &c__0, &nb);
	    t1 = second_();
	    i__2 = *lwork - iwork + 1;
...
/*        Generate right bidiagonalizing vectors of R in WORK(IL)   
	    ...
	    nb = ilaenv_(&c__1, "SORGBR", "P", m, m, m, &c_n1, (ftnlen)6, (
		    ftnlen)1);
	    lstime_1.opcnt[orgbr - 1] += sopla2_("SORGBR", "P", m, m, m, &
		    c__0, &nb);
	    t1 = second_();
	    i__2 = *lwork - iwork + 1;
...
/*        Multiply B by right singular vectors of L in WORK(IL)   
		...
		lstime_1.opcnt[gemm - 1] += sopbl3_("SGEMM ", m, nrhs, m);
		t1 = second_();
		sgemm_("T", "N", m, nrhs, m, &c_b145, &work[il], &ldwork, &b[
...
/* Computing MIN */
		    i__3 = *nrhs - i__ + 1;
		    bl = min(i__3,chunk);
		    lstime_1.opcnt[gemm - 1] += sopbl3_("SGEMM ", m, &bl, m);
		    t1 = second_();
		    sgemm_("T", "N", m, &bl, m, &c_b145, &work[il], &ldwork, &
...
/* L40: */
		...
		lstime_1.opcnt[gemv - 1] += sopbl2_("SGEMV ", m, m, &c__0, &
			c__0);
		t1 = second_();
		sgemv_("T", m, m, &c_b145, &work[il], &ldwork, &b_ref(1, 1), &
...
/*        Multiply transpose(Q) by B   
	    ...
	    nb = ilaenv_(&c__1, "SORMLQ", "LT", n, nrhs, m, &c_n1, (ftnlen)6, 
		    (ftnlen)2);
	    lstime_1.opcnt[ormlq - 1] += sopla_("SORMLQ", n, nrhs, m, &c__0, &
		    nb);
	    t1 = second_();
	    i__1 = *lwork - iwork + 1;
...
/*        Bidiagonalize A   
	    ...
	    nb = ilaenv_(&c__1, "SGEBRD", " ", m, n, &c_n1, &c_n1, (ftnlen)6, 
		    (ftnlen)1);
	    lstime_1.opcnt[gebrd - 1] += sopla_("SGEBRD", m, n, &c__0, &c__0, 
		    &nb);
	    t1 = second_();
	    i__1 = *lwork - iwork + 1;
...
/*        Multiply B by transpose of left bidiagonalizing vectors   
	    ...
	    nb = ilaenv_(&c__1, "SORMBR", "QLT", m, nrhs, n, &c_n1, (ftnlen)6,
		     (ftnlen)3);
	    lstime_1.opcnt[ormbr - 1] += sopla2_("SORMBR", "QLT", m, nrhs, n, 
		    &c__0, &nb);
	    t1 = second_();
	    i__1 = *lwork - iwork + 1;
...
/*        Generate right bidiagonalizing vectors in A   
	    ...
	    nb = ilaenv_(&c__1, "SORGBR", "P", m, n, m, &c_n1, (ftnlen)6, (
		    ftnlen)1);
	    lstime_1.opcnt[orgbr - 1] += sopla2_("SORGBR", "P", m, n, m, &
		    c__0, &nb);
	    t1 = second_();
	    i__1 = *lwork - iwork + 1;
...
/*        Multiply B by right singular vectors of A   
	    ...
	    if (*lwork >= *ldb * *nrhs && *nrhs > 1) {
		lstime_1.opcnt[gemm - 1] += sopbl3_("SGEMM ", n, nrhs, m);
		t1 = second_();
		sgemm_("T", "N", n, nrhs, m, &c_b145, &a[a_offset], lda, &b[
			b_offset], ldb, &c_b74, &work[1], ldb);
		t2 = second_();
		lstime_1.timng[gemm - 1] += t2 - t1;
...
/* Computing MIN */
		    i__3 = *nrhs - i__ + 1;
		    bl = min(i__3,chunk);
		    lstime_1.opcnt[gemm - 1] += sopbl3_("SGEMM ", n, &bl, m);
		    t1 = second_();
		    sgemm_("T", "N", n, &bl, m, &c_b145, &a[a_offset], lda, &
			    b_ref(1, i__), ldb, &c_b74, &work[1], n);
		    t2 = second_();
		    lstime_1.timng[gemm - 1] += t2 - t1;
...
/* L60: */
		...
		lstime_1.opcnt[gemv - 1] += sopbl2_("SGEMV ", m, n, &c__0, &
			c__0);
		t1 = second_();
		sgemv_("T", m, n, &c_b145, &a[a_offset], lda, &b[b_offset], &
...
/*     Undo scaling */
    ...
    if (iascl == 1) {
	lstime_1.opcnt[gelss - 1] += (real) (*n * *nrhs + minmn);
    ...
    if (ibscl == 1) {
	lstime_1.opcnt[gelss - 1] += (real) (*n * *nrhs);
