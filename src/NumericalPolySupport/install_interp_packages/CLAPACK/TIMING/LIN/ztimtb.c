...
/* Subroutine */ int ztimtb_(char *line, integer *nn, integer *nval, integer *
	...
	integer *nout, ftnlen line_len)
{
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, integer *, integer *), ztbtrs_(char *,
...
/*                 Time ZTBTRS */
			...
			for (i__ = 1; i__ <= i__4; ++i__) {
			    ...
			    s1 = dsecnd_();
L10:
			    ztbtrs_(uplo, "No transpose", "Non-unit", &n, &k, 
...
L20:
			    ...
			    ops = dopla_("ZTBTRS", &n, &nrhs, &k, &k, &c__0);
			    reslts_ref(i__, ik, i3, 1) = dmflop_(&ops, &time, 
				    &info);
/* L30: */
			}
		    }
...
/*           Print header for routine names. */
	    ...
	    if (in == 1 || s_cmp(cname, "ZTB   ", (ftnlen)6, (ftnlen)6) == 0) 
		    {
		io___29.ciunit = *nout;
		s_wsfe(&io___29);
