...
/* Subroutine */ int ztimqr_(char *line, integer *nm, integer *mval, integer *
	...
	ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "ZGEQRF" "ZUNGQR" "ZUNMQR";
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    ...
	    integer *, integer *), zgeqrf_(integer *, integer *, 
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    ...
	    *, doublecomplex *, integer *, doublecomplex *, integer *), zungqr_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, doublecomplex *, doublecomplex *, 
...
/*                 ZGEQRF:  QR factorization */
		    ...
		    s1 = dsecnd_();
L10:
		    zgeqrf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &
			    info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L20:
		    ...
		    ops = dopla_("ZGEQRF", &m, &n, &c__0, &c__0, &nb);
...
/*                 ZUNGQR:  Generate orthogonal matrix Q from the QR   
		    ...
		    s1 = dsecnd_();
L30:
		    zungqr_(&m, &minmn, &minmn, &b[1], &lda, &tau[1], &work[1]
			    , &lw, &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L40:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("ZUNGQR", &m, &minmn, &minmn, &c__0, &nb);
...
/* Computing MAX */
		...
		lw = max(i__3,i__4);
		zgeqrf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &info);
...
/*                       Do first for TRANS = 'N', then for TRANS = 'T' */
				...
				*(unsigned char *)trans = *(unsigned char *)&
				...
				s1 = dsecnd_();
L110:
				zunmqr_(side, trans, &m1, &n1, &k1, &a[1], &
...
L120:
				...
				time = (time - untime) / (doublereal) ic;
				i__5 = iside - 1;
				ops = dopla_("ZUNMQR", &m1, &n1, &k1, &i__5, &
					nb);
