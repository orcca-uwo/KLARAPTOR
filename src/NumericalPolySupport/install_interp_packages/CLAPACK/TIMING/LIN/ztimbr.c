...
/* Subroutine */ int ztimbr_(char *line, integer *nm, integer *mval, integer *
	nval, integer *nk, integer *kval, integer *nnb, integer *nbval, 
	integer *nxval, integer *nlda, integer *ldaval, doublereal *timmin, 
	doublecomplex *a, doublecomplex *b, doublereal *d__, doublecomplex *
	tau, doublecomplex *work, doublereal *reslts, integer *ldr1, integer *
	ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char subnam[6*3] = "ZGEBRD" "ZUNGBR" "ZUNMBR";
    ...
    static integer ic, nb, ik, im, nq;
    extern doublereal dsecnd_(void);
    static integer lw, nx, reseed[4];
    extern /* Subroutine */ int atimck_(integer *, char *, integer *, integer 
	    *, integer *, integer *, integer *, integer *, ftnlen), zgebrd_(
...
/*              Generate a test matrix of size M by N. */
		...
		zlatms_(&m, &n, "Uniform", iseed, "Nonsymm", &d__[1], &c__3, &
...
/*                 ZGEBRD:  Block reduction to bidiagonal form */
		    ...
		    s1 = dsecnd_();
L10:
		    zgebrd_(&m, &n, &a[1], &lda, &d__[1], &d__[minmn], &tau[1]
...
L20:
		    ...
		    ops = dopla_("ZGEBRD", &m, &n, &c__0, &c__0, &nb);
...
/* Computing MAX */
			...
			s1 = dsecnd_();
L30:
			zungbr_(vect, &m1, &n1, &k1, &b[1], &lda, &tau[1], &
				work[1], &lw, &info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
...
/*                    Op count for ZUNGBR: */
			...
			if (ivect == 1) {
			    if (m1 >= k1) {
				ops = dopla_("ZUNGQR", &m1, &n1, &k1, &c_n1, &
					nb);
			    } else {
				i__4 = m1 - 1;
				i__5 = m1 - 1;
				i__6 = m1 - 1;
				ops = dopla_("ZUNGQR", &i__4, &i__5, &i__6, &
					c_n1, &nb);
			    }
			} else {
			    if (k1 < n1) {
				ops = dopla_("ZUNGLQ", &m1, &n1, &k1, &c_n1, &
					nb);
			    } else {
				i__4 = n1 - 1;
				i__5 = n1 - 1;
				i__6 = n1 - 1;
				ops = dopla_("ZUNGLQ", &i__4, &i__5, &i__6, &
					c_n1, &nb);
			    }
			}
...
/* Computing MAX */
				...
				for (itran = 1; itran <= 2; ++itran) {
				    *(unsigned char *)trans = *(unsigned char 
				    ...
				    s1 = dsecnd_();
L60:
				    zunmbr_(vect, side, trans, &m1, &n1, &k1, 
...
/*                                Op count for ZUNMBR, VECT = 'P': */
					...
					if (nq >= k1) {
					    ...
					    i__6 = nq - 1;
					    i__7 = iside - 1;
					    ops = dopla_("ZUNMLQ", &i__5, &n1,
						     &i__6, &i__7, &nb);
					} else {
					    i__5 = n1 - 1;
					    i__6 = nq - 1;
					    i__7 = iside - 1;
					    ops = dopla_("ZUNMLQ", &m1, &i__5,
						     &i__6, &i__7, &nb);
					}
				    }
