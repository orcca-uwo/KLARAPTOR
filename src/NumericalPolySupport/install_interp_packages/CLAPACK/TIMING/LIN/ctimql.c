...
/* Subroutine */ int ctimql_(char *line, integer *nm, integer *mval, integer *
	...
	ftnlen line_len)
{
    ...
    static char subnam[6*3] = "CGEQLF" "CUNGQL" "CUNMQL";
    ...
    extern /* Subroutine */ int clacpy_(char *, integer *, integer *, complex 
	    *, integer *, complex *, integer *), ctimmg_(integer *, 
	    ...
	    integer *), cungql_(integer *, integer *, 
	    integer *, complex *, integer *, complex *, complex *, integer *, 
	    integer *), xlaenv_(integer *, integer *), cunmql_(char *, char *,
...
/*                 CGEQLF:  QL factorization */
		    ...
		    s1 = second_();
L10:
		    cgeqlf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &
			    info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
...
L20:
		    ...
		    ops = sopla_("CGEQLF", &m, &n, &c__0, &c__0, &nb);
...
/*                 CUNGQL:  Generate orthogonal matrix Q from the QL   
		    ...
		    s1 = second_();
L30:
		    cungql_(&m, &minmn, &minmn, &b[1], &lda, &tau[1], &work[1]
			    , &lw, &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
...
L40:
		    ...
		    time = (time - untime) / (real) ic;
		    ops = sopla_("CUNGQL", &m, &minmn, &minmn, &c__0, &nb);
...
/* Computing MAX */
		...
		lw = max(i__3,i__4);
		cgeqlf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &info);
...
/*                       Do first for TRANS = 'N', then for TRANS = 'T' */
				...
				*(unsigned char *)trans = *(unsigned char *)&
				...
				s1 = second_();
L110:
				cunmql_(side, trans, &m1, &n1, &k1, &a[1], &
...
L120:
				...
				time = (time - untime) / (real) ic;
				i__5 = iside - 1;
				ops = sopla_("CUNMQL", &m1, &n1, &k1, &i__5, &
					nb);
