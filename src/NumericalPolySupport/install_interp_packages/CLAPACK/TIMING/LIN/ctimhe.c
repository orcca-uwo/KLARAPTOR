...
/* Subroutine */ int ctimhe_(char *line, integer *nn, integer *nval, integer *
	...
	ldr3, integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "CHETRF" "CHETRS" "CHETRI";
    ...
    extern /* Subroutine */ int chetrf_(char *, integer *, complex *, integer 
	    ...
	    *, integer *, complex *, integer *), ctimmg_(integer *, 
	    ...
	    integer *, ftnlen, ftnlen, ftnlen), chetrs_(char *, integer *, 
...
/*                    Time CHETRF */
			...
			s1 = second_();
L10:
			chetrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &
				lwork, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L20:
			...
			ops = sopla_("CHETRF", &n, &n, &c__0, &c__0, &nb);
...
/*                 If CHETRF was not timed, generate a matrix and   
		    ...
		    nb = 1;
		    xlaenv_(&c__1, &nb);
		    chetrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &lwork, &
			    info);
		}
...
/*              Time CHETRI */
		...
		if (timsub[2]) {
		    ...
		    s1 = second_();
L40:
		    chetri_(uplo, &n, &b[1], &lda, &iwork[1], &work[1], &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
		    if (time < *timmin) {
...
L50:
		    ...
		    time = (time - untime) / (real) ic;
		    ops = sopla_("CHETRI", &n, &n, &c__0, &c__0, &c__0);
...
/*              Time CHETRS */
		...
		if (timsub[1]) {
			...
			s1 = second_();
L60:
			chetrs_(uplo, &n, &nrhs, &a[1], &lda, &iwork[1], &b[1]
				, &ldb, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L70:
			...
			time = (time - untime) / (real) ic;
			ops = sopla_("CHETRS", &n, &nrhs, &c__0, &c__0, &c__0);
