...
/* Subroutine */ int dtimgb_(char *line, integer *nm, integer *mval, integer *
	...
	line_len)
{
    ...
    static char subnam[6*2] = "DGBTRF" "DGBTRS";
    ...
    extern doublereal dopgb_(char *, integer *, integer *, integer *, integer 
	    *, integer *), dopla_(char *, integer *, integer *, 
	    integer *, integer *, integer *);
    static doublereal s1, s2;
    static integer ic, nb, ik, im, kl, ku;
    extern doublereal dsecnd_(void);
    extern /* Subroutine */ int atimck_(integer *, char *, integer *, integer 
	    *, integer *, integer *, integer *, integer *, ftnlen), dgbtrf_(
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    ...
	    *, integer *), dgbtrs_(char *, integer *, integer *, integer *, 
	    ...
	    integer *, integer *, integer *, integer *, doublereal *, integer 
	    *, integer *, integer *, ftnlen, ftnlen), xlaenv_(integer *, 
	    integer *);
...
/*                 Do for each value of NB in NBVAL.  Only DGBTRF is   
			...
			xlaenv_(&c__1, &nb);
			ic = 0;
			dtimmg_(&c__2, &m, &n, &a[1], &lda, &kl, &ku);
			s1 = dsecnd_();
L10:
			dgbtrf_(&m, &n, &kl, &ku, &a[1], &lda, &iwork[1], &
				info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
...
L20:
			...
			ops = dopgb_("DGBTRF", &m, &n, &kl, &ku, &iwork[1]);
			reslts_ref(inb, ik, ilda, 1) = dmflop_(&ops, &time, &
				info);
/* L30: */
		    }
		} else {
...
/*              Generate another matrix and factor it using DGBTRF so   
		...
		xlaenv_(&c__1, &nb);
		if (ic != 1) {
		    dgbtrf_(&m, &n, &kl, &ku, &a[1], &lda, &iwork[1], &info);
		}
...
/*              Time DGBTRS */
		...
		if (timsub[1]) {
			...
			s1 = dsecnd_();
L40:
			dgbtrs_("No transpose", &n, &kl, &ku, &nrhs, &a[1], &
...
L50:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("DGBTRS", &n, &nrhs, &kl, &ku, &c__0);
			reslts_ref(i__, ik, ilda, 2) = dmflop_(&ops, &time, &
				info);
/* L60: */
		    }
		}
...
/*           Print header for routine names. */
	    ...
	    if (im == 1 || s_cmp(cname, "DGB   ", (ftnlen)6, (ftnlen)6) == 0) 
		    {
		io___29.ciunit = *nout;
		s_wsfe(&io___29);
