...
/* Subroutine */ int dtimqr_(char *line, integer *nm, integer *mval, integer *
	...
	integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "DGEQRF" "DORGQR" "DORMQR";
    ...
    extern /* Subroutine */ int atimck_(integer *, char *, integer *, integer 
	    *, integer *, integer *, integer *, integer *, ftnlen), dgeqrf_(
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    ...
	    *, integer *), dorgqr_(integer *, integer 
	    *, integer *, doublereal *, integer *, doublereal *, doublereal *,
	     integer *, integer *), xlaenv_(integer *, integer *);
...
/*                 DGEQRF:  QR factorization */
		    ...
		    s1 = dsecnd_();
L10:
		    dgeqrf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &
			    info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L20:
		    ...
		    ops = dopla_("DGEQRF", &m, &n, &c__0, &c__0, &nb);
...
/*                 DORGQR:  Generate orthogonal matrix Q from the QR   
		    ...
		    s1 = dsecnd_();
L30:
		    dorgqr_(&m, &minmn, &minmn, &b[1], &lda, &tau[1], &work[1]
			    , &lw, &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L40:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("DORGQR", &m, &minmn, &minmn, &c__0, &nb);
...
/* Computing MAX */
		...
		lw = max(i__3,i__4);
		dgeqrf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &info);
...
/*                       Do first for TRANS = 'N', then for TRANS = 'T' */
				...
				*(unsigned char *)trans = *(unsigned char *)&
				...
				s1 = dsecnd_();
L110:
				dormqr_(side, trans, &m1, &n1, &k1, &a[1], &
...
L120:
				...
				time = (time - untime) / (doublereal) ic;
				i__5 = iside - 1;
				ops = dopla_("DORMQR", &m1, &n1, &k1, &i__5, &
					nb);
