...
/* Subroutine */ int dtimtr_(char *line, integer *nn, integer *nval, integer *
	...
	integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*2] = "DTRTRI" "DTRTRS";
    ...
    extern /* Subroutine */ int dtrtri_(char *, char *, integer *, doublereal 
	    *, integer *, integer *), dtrtrs_(char *, char *, 
...
/*                    Time DTRTRI */
			...
			s1 = dsecnd_();
L10:
			dtrtri_(uplo, "Non-unit", &n, &a[1], &lda, &info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
			if (time < *timmin) {
...
L20:
			...
			ops = dopla_("DTRTRI", &n, &n, &c__0, &c__0, &nb);
...
/*              Time DTRTRS */
		...
		if (timsub[1]) {
			...
			s1 = dsecnd_();
L40:
			dtrtrs_(uplo, "No transpose", "Non-unit", &n, &nrhs, &
...
L50:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("DTRTRS", &n, &nrhs, &c__0, &c__0, &c__0);
