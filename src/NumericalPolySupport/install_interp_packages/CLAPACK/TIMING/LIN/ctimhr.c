...
/* Subroutine */ int ctimhr_(char *line, integer *nm, integer *mval, integer *
	nn, integer *nval, integer *nnb, integer *nbval, integer *nxval, 
	integer *nlda, integer *ldaval, real *timmin, complex *a, complex *
	tau, complex *b, complex *work, real *rwork, real *reslts, integer *
	ldr1, integer *ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char subnam[6*3] = "CGEHRD" "CUNGHR" "CUNMHR";
    ...
    extern /* Subroutine */ int clacpy_(char *, integer *, integer *, complex 
	    *, integer *, complex *, integer *), ctimmg_(integer *, 
	    ...
	    integer *), cunghr_(integer *, integer *, 
	    integer *, complex *, integer *, complex *, complex *, integer *, 
	    integer *), cunmhr_(char *, char *, integer *, integer *, integer 
	    *, integer *, complex *, integer *, complex *, complex *, integer 
	    *, complex *, integer *, integer *), xlaenv_(
	    integer *, integer *);
...
/*              Generate a test matrix of size M by M. */
		...
		clatms_(&m, &m, "Uniform", iseed, "Nonsymm", &rwork[1], &c__3,
...
/*                 CGEHRD:  Reduction to Hesenberg form */
		    ...
		    clacpy_("Full", &m, &m, &b[1], &lda, &a[1], &lda);
		    ic = 0;
		    s1 = second_();
L10:
		    cgehrd_(&m, &ilo, &ihi, &a[1], &lda, &tau[1], &work[1], &
			    lw, &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
...
L20:
		    ...
		    ++icl;
		    if (icl <= ic) {
			clacpy_("Full", &m, &m, &a[1], &lda, &b[1], &lda);
			goto L20;
		    }

		    time = (time - untime) / (real) ic;
		    ops = sopla_("CGEHRD", &m, &ilo, &ihi, &c__0, &nb);
...
/*                 CUNGHR:  Generate the orthogonal matrix Q from the   
		    ...
		    s1 = second_();
L30:
		    cunghr_(&m, &ilo, &ihi, &b[1], &lda, &tau[1], &work[1], &
			    lw, &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
...
/*                 Op count for CUNGHR:  same as   
		    ...
		    i__5 = ihi - ilo;
		    i__6 = ihi - ilo;
		    ops = sopla_("CUNGQR", &i__4, &i__5, &i__6, &c__0, &nb);
...
/* Computing MAX */
				...
				*(unsigned char *)trans = *(unsigned char *)&
				...
				s1 = second_();
L50:
				cunmhr_(side, trans, &m1, &n1, &ilo, &ihi, &a[
...
/*                          Op count for CUNMHR, SIDE='L':  same as   
				...
				if (iside == 1) {
				    i__5 = ihi - ilo;
				    i__6 = ihi - ilo;
				    ops = sopla_("CUNMQR", &i__5, &n1, &i__6, 
					    &c_n1, &nb);
				} else {
				    i__5 = ihi - ilo;
				    i__6 = ihi - ilo;
				    ops = sopla_("CUNMQR", &m1, &i__5, &i__6, 
					    &c__1, &nb);
				}
...
/* L130: */
	...
	s_wsfe(&io___43);
	e_wsfe();
	sprtb3_("(  NB,  NX)", "N", nnb, &nbval[1], &nxval[1], nm, &mval[1], 
