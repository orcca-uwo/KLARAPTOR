...
/* Subroutine */ int dtimrq_(char *line, integer *nm, integer *mval, integer *
	...
	integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "DGERQF" "DORGRQ" "DORMRQ";
    ...
    extern /* Subroutine */ int atimck_(integer *, char *, integer *, integer 
	    *, integer *, integer *, integer *, integer *, ftnlen), dgerqf_(
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    ...
	    *, integer *), dorgrq_(integer *, integer 
...
/*                 DGERQF:  RQ factorization */
		    ...
		    s1 = dsecnd_();
L10:
		    dgerqf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &
			    info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L20:
		    ...
		    ops = dopla_("DGERQF", &m, &n, &c__0, &c__0, &nb);
...
/*                 DORGRQ:  Generate orthogonal matrix Q from the RQ   
		    ...
		    s1 = dsecnd_();
L30:
		    dorgrq_(&minmn, &n, &minmn, &b[1], &lda, &tau[1], &work[1]
			    , &lw, &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L40:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("DORGRQ", &minmn, &n, &minmn, &c__0, &nb);
...
/* Computing MAX */
		...
		lw = max(i__3,i__4);
		dgerqf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &info);
...
/*                       Do first for TRANS = 'N', then for TRANS = 'T' */
				...
				*(unsigned char *)trans = *(unsigned char *)&
				...
				s1 = dsecnd_();
L110:
				dormrq_(side, trans, &m1, &n1, &k1, &a[1], &
...
L120:
				...
				time = (time - untime) / (doublereal) ic;
				i__5 = iside - 1;
				ops = dopla_("DORMRQ", &m1, &n1, &k1, &i__5, &
					nb);
