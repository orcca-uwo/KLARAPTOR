...
/* Subroutine */ int ztimtr_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *nnb, integer *nbval, integer *nlda, 
	integer *ldaval, doublereal *timmin, doublecomplex *a, doublecomplex *
	b, doublereal *reslts, integer *ldr1, integer *ldr2, integer *ldr3, 
	integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char subnam[6*2] = "ZTRTRI" "ZTRTRS";
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, integer *, integer *), ztrtri_(char *,
	     char *, integer *, doublecomplex *, integer *, integer *), ztrtrs_(char *, char *, char *, integer *, integer *, 
...
/*                    Time ZTRTRI */
			...
			s1 = dsecnd_();
L10:
			ztrtri_(uplo, "Non-unit", &n, &a[1], &lda, &info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
			if (time < *timmin) {
...
L20:
			...
			ops = dopla_("ZTRTRI", &n, &n, &c__0, &c__0, &nb);
...
/*              Time ZTRTRS */
		...
		if (timsub[1]) {
			...
			s1 = dsecnd_();
L40:
			ztrtrs_(uplo, "No transpose", "Non-unit", &n, &nrhs, &
...
L50:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("ZTRTRS", &n, &nrhs, &c__0, &c__0, &c__0);
