...
/* Subroutine */ int ctimmv_(char *vname, integer *nn, integer *nval, integer 
	*nk, integer *kval, integer *nlda, integer *ldaval, real *timmin, 
	complex *a, integer *lb, complex *b, complex *c__, real *reslts, 
	integer *ldr1, integer *ldr2, integer *nout, ftnlen vname_len)
{
    /* Initialized data */

    static char subnam[6*2] = "CGEMV " "CGBMV ";
...
/*     Time CGEMV */
	...
	for (ilda = 1; ilda <= i__1; ++ilda) {
		...
		nrhs = n;
		ldb = lda;
		ctimmg_(&c__1, &n, &n, &a[1], &lda, &c__0, &c__0);
		ctimmg_(&c__0, &n, &nrhs, &b[1], &ldb, &c__0, &c__0);
		ctimmg_(&c__1, &n, &nrhs, &c__[1], &ldb, &c__0, &c__0);
		ic = 0;
		s1 = second_();
L40:
		ib = 1;
		...
		for (i__ = 1; i__ <= i__3; ++i__) {
		    cgemv_("No transpose", &n, &n, &c_b1, &a[1], &lda, &b[ib],
...
L60:
		...
		ops = nrhs * sopbl2_("CGEMV ", &n, &n, &c__0, &c__0);
...
/* L80: */
	...
	sprtbl_(lab1, lab2, &c__1, &nval[1], nn, &nval[1], nlda, &reslts[
...
/* Computing MIN   
   Computing MAX */
		    ...
		    ku = k;
		    ldb = n;
		    ctimmg_(&c__2, &n, &n, &a[1], &lda, &kl, &ku);
/* Computing MIN */
		    i__4 = k, i__5 = *lb / ldb;
		    nrhs = min(i__4,i__5);
		    ctimmg_(&c__0, &n, &nrhs, &b[1], &ldb, &c__0, &c__0);
...
L90:
			...
			cgbmv_("No transpose", &n, &n, &kl, &ku, &c_b1, &a[ku 
...
L110:
		    ...
		    time = (time - untime) / (real) ic;
		    ops = nrhs * sopbl2_("CGBMV ", &n, &n, &kl, &ku);
		    reslts_ref(in, ik, ilda) = smflop_(&ops, &time, &c__0);
/* L120: */
		}
/* L130: */
	    }
/* L140: */
	...
	sprtbl_(lab1, lab2, nn, &nval[1], nk, &kval[1], nlda, &reslts[
		reslts_offset], ldr1, ldr2, nout, (ftnlen)1, (ftnlen)1);
    }
