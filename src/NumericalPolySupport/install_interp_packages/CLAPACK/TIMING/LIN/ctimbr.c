	...
	line_len)
{
    ...
    extern /* Subroutine */ int cgebrd_(integer *, integer *, complex *, 
	    ...
	    integer *, integer *);
    static integer nq, lw, nx, reseed[4];
    ...
    extern /* Subroutine */ int clacpy_(char *, integer *, integer *, complex 
	    ...
	    *, integer *, char *, logical *, integer *, integer *, ftnlen, 
	    ftnlen, ftnlen), ctimmg_(integer *, integer *, integer *, complex 
	    *, integer *, integer *, integer *), clatms_(integer *, integer *,
	     char *, integer *, char *, real *, integer *, real *, real *, 
	    integer *, integer *, char *, complex *, integer *, complex *, 
	    integer *), cungbr_(char *, integer *, 
	    integer *, integer *, complex *, integer *, complex *, complex *, 
	    integer *, integer *), cunmbr_(char *, char *, char *, 
	    integer *, integer *, integer *, complex *, integer *, complex *, 
	    complex *, integer *, complex *, integer *, integer *), xlaenv_(integer *, integer *);
...
/*              Generate a test matrix of size M by N. */
		...
		clatms_(&m, &n, "Uniform", iseed, "Nonsymm", &d__[1], &c__3, &
...
/*                 CGEBRD:  Block reduction to bidiagonal form */
		    ...
		    s1 = second_();
L10:
		    cgebrd_(&m, &n, &a[1], &lda, &d__[1], &d__[minmn], &tau[1]
...
L20:
		    ...
		    ops = sopla_("CGEBRD", &m, &n, &c__0, &c__0, &nb);
...
/* Computing MAX */
			...
			s1 = second_();
L30:
			cungbr_(vect, &m1, &n1, &k1, &b[1], &lda, &tau[1], &
				work[1], &lw, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
/*                    Op count for CUNGBR: */
			...
			if (ivect == 1) {
			    if (m1 >= k1) {
				ops = sopla_("CUNGQR", &m1, &n1, &k1, &c_n1, &
					nb);
			    } else {
				i__4 = m1 - 1;
				i__5 = m1 - 1;
				i__6 = m1 - 1;
				ops = sopla_("CUNGQR", &i__4, &i__5, &i__6, &
					c_n1, &nb);
			    }
			} else {
			    if (k1 < n1) {
				ops = sopla_("CUNGLQ", &m1, &n1, &k1, &c_n1, &
					nb);
			    } else {
				i__4 = n1 - 1;
				i__5 = n1 - 1;
				i__6 = n1 - 1;
				ops = sopla_("CUNGLQ", &i__4, &i__5, &i__6, &
					c_n1, &nb);
			    }
			}
...
/* Computing MAX */
				...
				for (itran = 1; itran <= 2; ++itran) {
				    *(unsigned char *)trans = *(unsigned char 
				    ...
				    s1 = second_();
L60:
				    cunmbr_(vect, side, trans, &m1, &n1, &k1, 
...
/*                                Op count for CUNMBR, VECT = 'P': */
					...
					if (nq >= k1) {
					    ...
					    i__6 = nq - 1;
					    i__7 = iside - 1;
					    ops = sopla_("CUNMLQ", &i__5, &n1,
						     &i__6, &i__7, &nb);
					} else {
					    i__5 = n1 - 1;
					    i__6 = nq - 1;
					    i__7 = iside - 1;
					    ops = sopla_("CUNMLQ", &m1, &i__5,
						     &i__6, &i__7, &nb);
					}
				    }
