...
/* Subroutine */ int ctimpo_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *nnb, integer *nbval, integer *nlda, 
	integer *ldaval, real *timmin, complex *a, complex *b, integer *iwork,
	...
	nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "CPOTRF" "CPOTRS" "CPOTRI";
    ...
    static real untime;
    extern /* Subroutine */ int cpotri_(char *, integer *, complex *, integer 
	    *, integer *);
    static logical timsub[3];
    extern /* Subroutine */ int sprtbl_(char *, char *, integer *, integer *, 
	    integer *, integer *, integer *, real *, integer *, integer *, 
	    integer *, ftnlen, ftnlen), cpotrs_(char *, integer *, integer *, 
...
L20:
			...
			ops = sopla_("CPOTRF", &n, &n, &c__0, &c__0, &nb);
...
L40:
			...
			time = (time - untime) / (real) ic;
			ops = sopla_("CPOTRI", &n, &n, &c__0, &c__0, &nb);
			reslts_ref(inb, in, i3, 3) = smflop_(&ops, &time, &
				info);
		    }
/* L50: */
		}
...
/*              Time CPOTRS */
		...
		if (timsub[1]) {
			...
			s1 = second_();
L60:
			cpotrs_(uplo, &n, &nrhs, &a[1], &lda, &b[1], &ldb, &
				info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L70:
			...
			time = (time - untime) / (real) ic;
			ops = sopla_("CPOTRS", &n, &nrhs, &c__0, &c__0, &c__0);
