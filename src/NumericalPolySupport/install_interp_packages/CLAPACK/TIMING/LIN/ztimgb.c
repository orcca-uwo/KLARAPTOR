...
/* Subroutine */ int ztimgb_(char *line, integer *nm, integer *mval, integer *
	...
	ftnlen line_len)
{
    ...
    static char subnam[6*2] = "ZGBTRF" "ZGBTRS";
...
/*                 Do for each value of NB in NBVAL.  Only ZGBTRF is   
			...
			xlaenv_(&c__1, &nb);
			ic = 0;
			ztimmg_(&c__2, &m, &n, &a[1], &lda, &kl, &ku);
			s1 = dsecnd_();
L10:
			zgbtrf_(&m, &n, &kl, &ku, &a[1], &lda, &iwork[1], &
				info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
...
L20:
			...
			ops = dopgb_("ZGBTRF", &m, &n, &kl, &ku, &iwork[1]);
...
/*              Generate another matrix and factor it using ZGBTRF so   
		...
		xlaenv_(&c__1, &nb);
		if (ic != 1) {
		    zgbtrf_(&m, &n, &kl, &ku, &a[1], &lda, &iwork[1], &info);
		}
...
/*              Time ZGBTRS */
		...
		if (timsub[1]) {
			...
			s1 = dsecnd_();
L40:
			zgbtrs_("No transpose", &n, &kl, &ku, &nrhs, &a[1], &
...
L50:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("ZGBTRS", &n, &nrhs, &kl, &ku, &c__0);
...
/*           Print header for routine names. */
	    ...
	    if (im == 1 || s_cmp(cname, "ZGB   ", (ftnlen)6, (ftnlen)6) == 0) 
		    {
		io___29.ciunit = *nout;
		s_wsfe(&io___29);
