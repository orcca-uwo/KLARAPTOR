...
/* Subroutine */ int stimtb_(char *line, integer *nn, integer *nval, integer *
	...
	line_len)
{
    ...
    extern /* Subroutine */ int stimmg_(integer *, integer *, integer *, real 
	    *, integer *, integer *, integer *), sprtbl_(char *, char *, 
	    ...
	    integer *, integer *, integer *, ftnlen, ftnlen), stbtrs_(char *, 
...
/*                 Time STBTRS */
			...
			for (i__ = 1; i__ <= i__4; ++i__) {
			    ...
			    s1 = second_();
L10:
			    stbtrs_(uplo, "No transpose", "Non-unit", &n, &k, 
...
L20:
			    ...
			    ops = sopla_("STBTRS", &n, &nrhs, &k, &k, &c__0);
...
/*           Print header for routine names. */
	    ...
	    if (in == 1 || s_cmp(cname, "STB   ", (ftnlen)6, (ftnlen)6) == 0) 
		    {
		io___29.ciunit = *nout;
		s_wsfe(&io___29);
