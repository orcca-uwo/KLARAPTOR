...
/* Subroutine */ int ctimpb_(char *line, integer *nn, integer *nval, integer *
	...
	ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*2] = "CPBTRF" "CPBTRS";
    ...
    extern /* Subroutine */ int cpbtrf_(char *, integer *, integer *, complex 
	    *, integer *, integer *), atimin_(char *, char *, integer 
	    *, char *, logical *, integer *, integer *, ftnlen, ftnlen, 
	    ftnlen), ctimmg_(integer *, integer *, integer *, complex *, 
	    integer *, integer *, integer *), cpbtrs_(char *, integer *, 
...
/*                 Time CPBTRS */
			...
			for (i__ = 1; i__ <= i__4; ++i__) {
			    ...
			    s1 = second_();
L40:
			    cpbtrs_(uplo, &n, &k, &nrhs, &a[1], &lda, &b[1], &
				    ldb, &info);
			    s2 = second_();
			    time = s2 - s1;
			    ++ic;
...
L50:
			    ...
			    time = (time - untime) / (real) ic;
			    ops = sopla_("CPBTRS", &n, &nrhs, &k, &k, &c__0);
...
/*           Print header for routine names. */
	    ...
	    if (in == 1 || s_cmp(cname, "CPB   ", (ftnlen)6, (ftnlen)6) == 0) 
		    {
		io___31.ciunit = *nout;
		s_wsfe(&io___31);
