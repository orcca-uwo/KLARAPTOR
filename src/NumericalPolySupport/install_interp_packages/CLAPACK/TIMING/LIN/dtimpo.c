...
/* Subroutine */ int dtimpo_(char *line, integer *nn, integer *nval, integer *
	...
	integer *ldr3, integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "DPOTRF" "DPOTRS" "DPOTRI";
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    ...
	    *, integer *), dprtbl_(char *, char *, integer *, integer *, 
	    integer *, integer *, integer *, doublereal *, integer *, integer 
	    *, integer *, ftnlen, ftnlen), dpotrf_(char *, integer *, 
	    doublereal *, integer *, integer *), xlaenv_(integer *, 
	    integer *);
    static doublereal untime;
    extern /* Subroutine */ int dpotri_(char *, integer *, doublereal *, 
	    integer *, integer *);
...
L20:
			...
			ops = dopla_("DPOTRF", &n, &n, &c__0, &c__0, &nb);
...
L40:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("DPOTRI", &n, &n, &c__0, &c__0, &nb);
...
/*              Time DPOTRS */
		...
		if (timsub[1]) {
			...
			s1 = dsecnd_();
L60:
			dpotrs_(uplo, &n, &nrhs, &a[1], &lda, &b[1], &ldb, &
				info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
...
L70:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("DPOTRS", &n, &nrhs, &c__0, &c__0, &c__0);
