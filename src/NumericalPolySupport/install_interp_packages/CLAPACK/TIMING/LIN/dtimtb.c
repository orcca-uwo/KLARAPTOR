...
/* Subroutine */ int dtimtb_(char *line, integer *nn, integer *nval, integer *
	...
	integer *nout, ftnlen line_len)
{
    ...
    extern /* Subroutine */ int dtbtrs_(char *, char *, char *, integer *, 
...
/*                 Time DTBTRS */
			...
			for (i__ = 1; i__ <= i__4; ++i__) {
			    ...
			    s1 = dsecnd_();
L10:
			    dtbtrs_(uplo, "No transpose", "Non-unit", &n, &k, 
...
L20:
			    ...
			    ops = dopla_("DTBTRS", &n, &nrhs, &k, &k, &c__0);
...
/*           Print header for routine names. */
	    ...
	    if (in == 1 || s_cmp(cname, "DTB   ", (ftnlen)6, (ftnlen)6) == 0) 
		    {
		io___29.ciunit = *nout;
		s_wsfe(&io___29);
