...
/* Subroutine */ int ztimhp_(char *line, integer *nn, integer *nval, integer *
	...
	ftnlen line_len)
{
    ...
    static char subnam[6*3] = "ZHPTRF" "ZHPTRS" "ZHPTRI";
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, integer *, integer *), zhptrf_(char *,
	     integer *, doublecomplex *, integer *, integer *), 
	    zhptri_(char *, integer *, doublecomplex *, integer *, 
	    doublecomplex *, integer *), zhptrs_(char *, integer *, 
...
/*           Time ZHPTRF */
		...
		s1 = dsecnd_();
L10:
		zhptrf_(uplo, &n, &a[1], &iwork[1], &info);
		s2 = dsecnd_();
		time = s2 - s1;
		++ic;
		if (time < *timmin) {
...
L20:
		...
		ops = dopla_("ZHPTRF", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time ZHPTRI */
		...
		s1 = dsecnd_();
L30:
		zhptri_(uplo, &n, &b[1], &iwork[1], &work[1], &info);
		s2 = dsecnd_();
		time = s2 - s1;
		++ic;
		if (time < *timmin) {
...
L40:
		...
		time = (time - untime) / (doublereal) ic;
		ops = dopla_("ZHPTRI", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time ZHPTRS */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    s1 = dsecnd_();
L50:
		    zhptrs_(uplo, &n, &nrhs, &a[1], &iwork[1], &b[1], &ldb, &
			    info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L60:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("ZHPTRS", &n, &nrhs, &c__0, &c__0, &c__0);
