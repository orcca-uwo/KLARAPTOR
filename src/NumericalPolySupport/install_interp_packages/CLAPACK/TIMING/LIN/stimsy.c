...
/* Subroutine */ int stimsy_(char *line, integer *nn, integer *nval, integer *
	...
	integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "SSYTRF" "SSYTRS" "SSYTRI";
    ...
    extern /* Subroutine */ int sprtbl_(char *, char *, integer *, integer *, 
	    ...
	    integer *, ftnlen, ftnlen), ssytrf_(char *, integer *, real *, 
	    integer *, integer *, real *, integer *, integer *), 
	    ssytri_(char *, integer *, real *, integer *, integer *, real *, 
...
/* Computing MAX */
			...
			s1 = second_();
L10:
			ssytrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &
				lwork, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L20:
			...
			++icl;
			if (icl <= ic) {
			    stimmg_(&mat, &n, &n, &b[1], &lda, &c__0, &c__0);
			    goto L20;
			}

			time = (time - untime) / (real) ic;
			ops = sopla_("SSYTRF", &n, &n, &c__0, &c__0, &nb);
...
/*                 If SSYTRF was not timed, generate a matrix and   
		    ...
		    nb = 1;
		    xlaenv_(&c__1, &nb);
		    ssytrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &lwork, &
			    info);
		}
...
/*              Time SSYTRI */
		...
		if (timsub[2]) {
		    slacpy_(uplo, &n, &n, &a[1], &lda, &b[1], &lda)
			    ;
		    ic = 0;
		    s1 = second_();
L40:
		    ssytri_(uplo, &n, &b[1], &lda, &iwork[1], &work[1], &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
		    if (time < *timmin) {
...
L50:
		    ...
		    time = (time - untime) / (real) ic;
		    ops = sopla_("SSYTRI", &n, &n, &c__0, &c__0, &c__0);
...
/*              Time SSYTRS */
		...
		if (timsub[1]) {
			...
			s1 = second_();
L60:
			ssytrs_(uplo, &n, &nrhs, &a[1], &lda, &iwork[1], &b[1]
				, &ldb, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L70:
			...
			time = (time - untime) / (real) ic;
			ops = sopla_("SSYTRS", &n, &nrhs, &c__0, &c__0, &c__0);
