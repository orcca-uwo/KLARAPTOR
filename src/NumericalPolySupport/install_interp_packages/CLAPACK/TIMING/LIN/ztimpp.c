...
/* Subroutine */ int ztimpp_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *la, doublereal *timmin, doublecomplex *
	a, doublecomplex *b, integer *iwork, doublereal *reslts, integer *
	ldr1, integer *ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char uplos[1*2] = "U" "L";
    static char subnam[6*3] = "ZPPTRF" "ZPPTRS" "ZPPTRI";
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, integer *, integer *), zpptrf_(char *,
	     integer *, doublecomplex *, integer *), zpptri_(char *, 
	    integer *, doublecomplex *, integer *), zpptrs_(char *, 
...
L20:
		...
		ops = dopla_("ZPPTRF", &n, &n, &c__0, &c__0, &c__0)
			;
...
L40:
		...
		time = (time - untime) / (doublereal) ic;
		ops = dopla_("ZPPTRI", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time ZPPTRS */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    s1 = dsecnd_();
L50:
		    zpptrs_(uplo, &n, &nrhs, &a[1], &b[1], &ldb, &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
		    if (time < *timmin) {
...
L60:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("ZPPTRS", &n, &nrhs, &c__0, &c__0, &c__0);
