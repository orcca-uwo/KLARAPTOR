...
/* Subroutine */ int stimql_(char *line, integer *nm, integer *mval, integer *
	...
	*ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "SGEQLF" "SORGQL" "SORMQL";
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    logical *, integer *, integer *, ftnlen, ftnlen, ftnlen), sgeqlf_(
    ...
    extern /* Subroutine */ int slatms_(integer *, integer *, char *, integer 
	    *, char *, real *, integer *, real *, real *, integer *, integer *
	    , char *, real *, integer *, real *, integer *), sorgql_(integer *, integer *, integer *, real *, integer 
...
/*                 SGEQLF:  QL factorization */
		    ...
		    s1 = second_();
L10:
		    sgeqlf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &
			    info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
...
L20:
		    ...
		    ops = sopla_("SGEQLF", &m, &n, &c__0, &c__0, &nb);
...
/*                 SORGQL:  Generate orthogonal matrix Q from the QL   
		    ...
		    s1 = second_();
L30:
		    sorgql_(&m, &minmn, &minmn, &b[1], &lda, &tau[1], &work[1]
			    , &lw, &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
...
L40:
		    ...
		    time = (time - untime) / (real) ic;
		    ops = sopla_("SORGQL", &m, &minmn, &minmn, &c__0, &nb);
...
/* Computing MAX */
		...
		lw = max(i__3,i__4);
		sgeqlf_(&m, &n, &a[1], &lda, &tau[1], &work[1], &lw, &info);
...
/*                       Do first for TRANS = 'N', then for TRANS = 'T' */
				...
				*(unsigned char *)trans = *(unsigned char *)&
				...
				s1 = second_();
L110:
				sormql_(side, trans, &m1, &n1, &k1, &a[1], &
...
L120:
				...
				time = (time - untime) / (real) ic;
				i__5 = iside - 1;
				ops = sopla_("SORMQL", &m1, &n1, &k1, &i__5, &
					nb);
