...
/* Subroutine */ int stimtr_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *nnb, integer *nbval, integer *nlda, 
	integer *ldaval, real *timmin, real *a, real *b, real *reslts, 
	integer *ldr1, integer *ldr2, integer *ldr3, integer *nout, ftnlen 
	line_len)
{
    /* Initialized data */

    static char subnam[6*2] = "STRTRI" "STRTRS";
    ...
    extern /* Subroutine */ int sprtbl_(char *, char *, integer *, integer *, 
	    ...
	    integer *, ftnlen, ftnlen), strtri_(char *, char *, integer *, 
	    real *, integer *, integer *), strtrs_(char *, 
...
/*                    Time STRTRI */
			...
			s1 = second_();
L10:
			strtri_(uplo, "Non-unit", &n, &a[1], &lda, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
			if (time < *timmin) {
...
L20:
			...
			ops = sopla_("STRTRI", &n, &n, &c__0, &c__0, &nb);
...
/*              Time STRTRS */
		...
		if (timsub[1]) {
			...
			s1 = second_();
L40:
			strtrs_(uplo, "No transpose", "Non-unit", &n, &nrhs, &
...
L50:
			...
			time = (time - untime) / (real) ic;
			ops = sopla_("STRTRS", &n, &nrhs, &c__0, &c__0, &c__0);
