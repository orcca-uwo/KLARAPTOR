...
/* Subroutine */ int dtimtp_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *la, doublereal *timmin, doublereal *a, 
	doublereal *b, doublereal *reslts, integer *ldr1, integer *ldr2, 
	integer *ldr3, integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char subnam[6*2] = "DTPTRI" "DTPTRS";
    ...
    extern /* Subroutine */ int dtptri_(char *, char *, integer *, doublereal 
	    *, integer *), dtptrs_(char *, char *, char *, 
...
/*           Time DTPTRI */
		...
		s1 = dsecnd_();
L10:
		dtptri_(uplo, "Non-unit", &n, &a[1], &info);
		s2 = dsecnd_();
		time = s2 - s1;
		++ic;
		if (time < *timmin) {
...
L20:
		...
		ops = dopla_("DTPTRI", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time DTPTRS */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    s1 = dsecnd_();
L30:
		    dtptrs_(uplo, "No transpose", "Non-unit", &n, &nrhs, &a[1]
			    , &b[1], &ldb, &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L40:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("DTPTRS", &n, &nrhs, &c__0, &c__0, &c__0);
