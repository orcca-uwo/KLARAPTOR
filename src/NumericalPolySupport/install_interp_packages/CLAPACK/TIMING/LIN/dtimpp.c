...
/* Subroutine */ int dtimpp_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *la, doublereal *timmin, doublereal *a, 
	doublereal *b, integer *iwork, doublereal *reslts, integer *ldr1, 
	integer *ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char uplos[1*2] = "U" "L";
    static char subnam[6*3] = "DPPTRF" "DPPTRS" "DPPTRI";
    ...
    extern /* Subroutine */ int atimin_(char *, char *, integer *, char *, 
	    ...
	    *, integer *), dprtbl_(char *, char *, integer *, integer *, 
	    integer *, integer *, integer *, doublereal *, integer *, integer 
	    *, integer *, ftnlen, ftnlen), dpptrf_(char *, integer *, 
	    doublereal *, integer *);
    static doublereal untime;
    extern /* Subroutine */ int dpptri_(char *, integer *, doublereal *, 
	    integer *);
...
L20:
		...
		ops = dopla_("DPPTRF", &n, &n, &c__0, &c__0, &c__0)
			;
...
L40:
		...
		time = (time - untime) / (doublereal) ic;
		ops = dopla_("DPPTRI", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time DPPTRS */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    s1 = dsecnd_();
L50:
		    dpptrs_(uplo, &n, &nrhs, &a[1], &b[1], &ldb, &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
		    if (time < *timmin) {
...
L60:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("DPPTRS", &n, &nrhs, &c__0, &c__0, &c__0);
