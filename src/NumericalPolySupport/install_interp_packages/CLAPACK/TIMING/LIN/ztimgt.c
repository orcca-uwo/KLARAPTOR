...
/* Subroutine */ int ztimgt_(char *line, integer *nm, integer *mval, integer *
	nns, integer *nsval, integer *nlda, integer *ldaval, doublereal *
	timmin, doublecomplex *a, doublecomplex *b, integer *iwork, 
	doublereal *reslts, integer *ldr1, integer *ldr2, integer *ldr3, 
	integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char subnam[6*4] = "ZGTTRF" "ZGTTRS" "ZGTSV " "ZGTSL ";
    ...
    static char fmt_9998[] = "(\002 ZGTTRS with TRANS = '\002,a1,\002'\002,/)"
	    ;
    ...
    static char trans[1];
    extern /* Subroutine */ int zgtsl_(integer *, doublecomplex *, 
	    doublecomplex *, doublecomplex *, doublecomplex *, integer *);
    static doublereal s1, s2;
    extern /* Subroutine */ int zgtsv_(integer *, integer *, doublecomplex *, 
	    doublecomplex *, doublecomplex *, doublecomplex *, integer *, 
	    integer *);
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, integer *, integer *), zgttrf_(
...
/*        Time ZGTTRF */
	...
	if (timsub[0]) {
	    i__2 = n * 3;
	    ztimmg_(&c__14, &m, &m, &a[1], &i__2, &c__0, &c__0);
	    ic = 0;
	    s1 = dsecnd_();
L20:
	    zgttrf_(&m, &a[1], &a[n], &a[n * 2], &a[n * 3 - 2], &iwork[1], &
		    info);
	    s2 = dsecnd_();
	    time = s2 - s1;
	    ++ic;
...
L30:
	    ...
	    ops = dopgb_("ZGTTRF", &m, &m, &c__1, &c__1, &iwork[1])
		    ;
	...
	if (timsub[1]) {
		...
		*(unsigned char *)trans = *(unsigned char *)&transs[itrans - 
		...
		for (ilda = 1; ilda <= i__2; ++ilda) {
			...
			s1 = dsecnd_();
L40:
			zgttrs_(trans, &m, &nrhs, &a[1], &a[n], &a[n * 2], &a[
...
L50:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopgb_("ZGTTRS", &m, &nrhs, &c__0, &c__0, &
				iwork[1]);
			reslts_ref(i__, im, ilda, isub) = dmflop_(&ops, &time,
				 &info);
/* L60: */
		    }
/* L70: */
...
/* L80: */
	...
	if (timsub[2]) {
		...
		for (i__ = 1; i__ <= i__3; ++i__) {
		    nrhs = nsval[i__];
		    i__4 = n * 3;
		    ztimmg_(&c__14, &m, &m, &a[1], &i__4, &c__0, &c__0);
		    ztimmg_(&c__0, &m, &nrhs, &b[1], &ldb, &c__0, &c__0);
		    ic = 0;
		    s1 = dsecnd_();
L90:
		    zgtsv_(&m, &nrhs, &a[1], &a[n], &a[n * 2], &b[1], &ldb, &
			    info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L100:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopgb_("ZGTSV ", &m, &nrhs, &c__0, &c__0, &iwork[1]);
...
/* L120: */
	...
	if (timsub[3]) {
	    ...
	    s1 = dsecnd_();
L130:
	    zgtsl_(&m, &a[1], &a[n], &a[n * 2], &b[1], &info);
	    s2 = dsecnd_();
	    time = s2 - s1;
	    ++ic;
	    if (time < *timmin) {
		i__2 = n * 3;
		ztimmg_(&c__14, &m, &m, &a[1], &i__2, &c__0, &c__0);
		ztimmg_(&c__0, &m, &c__1, &b[1], &ldb, &c__0, &c__0);
		goto L130;
	    }
...
L140:
	    ...
	    time = (time - untime) / (doublereal) ic;
	    ops = dopgb_("ZGTSV ", &m, &c__1, &c__0, &c__0, &iwork[1]);
