...
/* Subroutine */ int ctimsy_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *nnb, integer *nbval, integer *nlda, 
	integer *ldaval, real *timmin, complex *a, complex *b, complex *work, 
	integer *iwork, real *reslts, integer *ldr1, integer *ldr2, integer *
	ldr3, integer *nout, ftnlen line_len)
{
    /* Initialized data */

    static char uplos[1*2] = "U" "L";
    static char subnam[6*3] = "CSYTRF" "CSYTRS" "CSYTRI";
    ...
    extern /* Subroutine */ int clacpy_(char *, integer *, integer *, complex 
	    *, integer *, complex *, integer *), ctimmg_(integer *, 
	    ...
	    integer *, ftnlen, ftnlen, ftnlen), xlaenv_(integer *, integer *);
    ...
    extern /* Subroutine */ int sprtbl_(char *, char *, integer *, integer *, 
	    ...
	    integer *, ftnlen, ftnlen), csytrf_(char *, integer *, complex *, 
...
/*                    Time CSYTRF */
			...
			s1 = second_();
L10:
			csytrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &
				lwork, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L20:
			...
			++icl;
			if (icl <= ic) {
			    ctimmg_(&mat, &n, &n, &b[1], &lda, &c__0, &c__0);
			    goto L20;
			}

			time = (time - untime) / (real) ic;
			ops = sopla_("CSYTRF", &n, &n, &c__0, &c__0, &nb);
...
/*                 If CSYTRF was not timed, generate a matrix and   
		    ...
		    nb = 1;
		    xlaenv_(&c__1, &nb);
		    csytrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &lwork, &
			    info);
		}
...
/*              Time CSYTRI */
		...
		if (timsub[2]) {
		    clacpy_(uplo, &n, &n, &a[1], &lda, &b[1], &lda)
			    ;
		    ic = 0;
		    s1 = second_();
L40:
		    csytri_(uplo, &n, &b[1], &lda, &iwork[1], &work[1], &info);
		    s2 = second_();
		    time = s2 - s1;
		    ++ic;
		    if (time < *timmin) {
...
L50:
		    ...
		    time = (time - untime) / (real) ic;
		    ops = sopla_("CSYTRI", &n, &n, &c__0, &c__0, &c__0);
		    reslts_ref(1, in, i3, 3) = smflop_(&ops, &time, &info);
		}
...
/*              Time CSYTRS */
		...
		if (timsub[1]) {
			...
			s1 = second_();
L60:
			csytrs_(uplo, &n, &nrhs, &a[1], &lda, &iwork[1], &b[1]
				, &ldb, &info);
			s2 = second_();
			time = s2 - s1;
			++ic;
...
L70:
			...
			time = (time - untime) / (real) ic;
			ops = sopla_("CSYTRS", &n, &nrhs, &c__0, &c__0, &c__0);
...
/* L120: */
	...
	for (iuplo = 1; iuplo <= 2; ++iuplo) {
		...
		sprtbl_(" ", "N", &c__1, &nbval[1], nn, &nval[1], nlda, &
