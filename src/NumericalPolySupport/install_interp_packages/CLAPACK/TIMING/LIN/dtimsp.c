...
/* Subroutine */ int dtimsp_(char *line, integer *nn, integer *nval, integer *
	nns, integer *nsval, integer *la, doublereal *timmin, doublereal *a, 
	doublereal *b, doublereal *work, integer *iwork, doublereal *reslts, 
	...
	line_len)
{
    ...
    static char subnam[6*3] = "DSPTRF" "DSPTRS" "DSPTRI";
    ...
    extern /* Subroutine */ int dsptri_(char *, integer *, doublereal *, 
	    integer *, doublereal *, integer *), dsptrs_(char *, 
...
/*           Time DSPTRF */
		...
		s1 = dsecnd_();
L10:
		dsptrf_(uplo, &n, &a[1], &iwork[1], &info);
		s2 = dsecnd_();
		time = s2 - s1;
		++ic;
		if (time < *timmin) {
...
L20:
		...
		ops = dopla_("DSPTRF", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time DSPTRI */
	    ...
	    if (timsub[2]) {
		dcopy_(&lda, &a[1], &c__1, &b[1], &c__1);
		ic = 0;
		s1 = dsecnd_();
L30:
		dsptri_(uplo, &n, &b[1], &iwork[1], &work[1], &info);
		s2 = dsecnd_();
		time = s2 - s1;
		++ic;
		if (time < *timmin) {
...
L40:
		...
		time = (time - untime) / (doublereal) ic;
		ops = dopla_("DSPTRI", &n, &n, &c__0, &c__0, &c__0)
			;
...
/*           Time DSPTRS */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    s1 = dsecnd_();
L50:
		    dsptrs_(uplo, &n, &nrhs, &a[1], &iwork[1], &b[1], &ldb, &
			    info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
...
L60:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("DSPTRS", &n, &nrhs, &c__0, &c__0, &c__0);
