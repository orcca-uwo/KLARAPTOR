...
/* Subroutine */ int ztimsy_(char *line, integer *nn, integer *nval, integer *
	...
	ldr1, integer *ldr2, integer *ldr3, integer *nout, ftnlen line_len)
{
    ...
    static char subnam[6*3] = "ZSYTRF" "ZSYTRS" "ZSYTRI";
    ...
    extern /* Subroutine */ int ztimmg_(integer *, integer *, integer *, 
	    doublecomplex *, integer *, integer *, integer *), zsytrf_(char *,
...
/*                    Time ZSYTRF */
			...
			s1 = dsecnd_();
L10:
			zsytrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &
				lwork, &info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
...
L20:
			...
			ops = dopla_("ZSYTRF", &n, &n, &c__0, &c__0, &nb);
...
/*                 If ZSYTRF was not timed, generate a matrix and   
		    ...
		    nb = 1;
		    xlaenv_(&c__1, &nb);
		    zsytrf_(uplo, &n, &a[1], &lda, &iwork[1], &b[1], &lwork, &
			    info);
		}
...
/*              Time ZSYTRI */
		...
		if (timsub[2]) {
		    ...
		    s1 = dsecnd_();
L40:
		    zsytri_(uplo, &n, &b[1], &lda, &iwork[1], &work[1], &info);
		    s2 = dsecnd_();
		    time = s2 - s1;
		    ++ic;
		    if (time < *timmin) {
...
L50:
		    ...
		    time = (time - untime) / (doublereal) ic;
		    ops = dopla_("ZSYTRI", &n, &n, &c__0, &c__0, &c__0);
...
/*              Time ZSYTRS */
		...
		if (timsub[1]) {
			...
			s1 = dsecnd_();
L60:
			zsytrs_(uplo, &n, &nrhs, &a[1], &lda, &iwork[1], &b[1]
				, &ldb, &info);
			s2 = dsecnd_();
			time = s2 - s1;
			++ic;
...
L70:
			...
			time = (time - untime) / (doublereal) ic;
			ops = dopla_("ZSYTRS", &n, &nrhs, &c__0, &c__0, &c__0);
