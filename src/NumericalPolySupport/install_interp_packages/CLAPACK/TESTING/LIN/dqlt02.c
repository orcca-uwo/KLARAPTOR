...
/*     Copy the last k columns of the factorization to the array Q */
    ...
    if (*k < *m) {
	i__1 = *m - *k;
	dlacpy_("Full", &i__1, k, &af_ref(1, *n - *k + 1), lda, &q_ref(1, *n 
		- *k + 1), lda);
    }
    if (*k > 1) {
	i__1 = *k - 1;
	i__2 = *k - 1;
	dlacpy_("Upper", &i__1, &i__2, &af_ref(*m - *k + 1, *n - *k + 2), lda,
...
/*     Copy L(m-n+1:m,n-k+1:n) */
    ...
    dlaset_("Full", n, k, &c_b10, &c_b10, &l_ref(*m - *n + 1, *n - *k + 1), 
	    lda);
    dlacpy_("Lower", k, k, &af_ref(*m - *k + 1, *n - *k + 1), lda, &l_ref(*m 
	    - *k + 1, *n - *k + 1), lda);
...
/*     Compute norm( L - Q'*A ) / ( M * norm(A) * EPS ) . */
    ...
    anorm = dlange_("1", m, k, &a_ref(1, *n - *k + 1), lda, &rwork[1]);
    resid = dlange_("1", n, k, &l_ref(*m - *n + 1, *n - *k + 1), lda, &rwork[
	    1]);
    if (anorm > 0.) {
