...
/*     Test 2:  Compute the maximum of BERR / ( NZ*EPS + (*) ), where   
    ...
    for (k = 1; k <= i__1; ++k) {
	if (*n == 1) {
	    axbi = (r__1 = b_ref(1, k), dabs(r__1)) + (r__2 = d__[1] * x_ref(
		    1, k), dabs(r__2));
	} else {
	    axbi = (r__1 = b_ref(1, k), dabs(r__1)) + (r__2 = d__[1] * x_ref(
		    1, k), dabs(r__2)) + (r__3 = e[1] * x_ref(2, k), dabs(
		    r__3));
	    i__2 = *n - 1;
	    for (i__ = 2; i__ <= i__2; ++i__) {
		tmp = (r__1 = b_ref(i__, k), dabs(r__1)) + (r__2 = e[i__ - 1] 
			* x_ref(i__ - 1, k), dabs(r__2)) + (r__3 = d__[i__] * 
			x_ref(i__, k), dabs(r__3)) + (r__4 = e[i__] * x_ref(
			i__ + 1, k), dabs(r__4));
		axbi = dmin(axbi,tmp);
/* L40: */
	    }
	    tmp = (r__1 = b_ref(*n, k), dabs(r__1)) + (r__2 = e[*n - 1] * 
		    x_ref(*n - 1, k), dabs(r__2)) + (r__3 = d__[*n] * x_ref(*
		    n, k), dabs(r__3));
	    axbi = dmin(axbi,tmp);
	}
/* Computing MAX */
