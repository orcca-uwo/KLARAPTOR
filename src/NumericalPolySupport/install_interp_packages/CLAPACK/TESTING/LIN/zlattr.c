	...
	info)
{
    ...
    static doublecomplex star1;
    static doublereal c__;
    static doublecomplex plus1, plus2;
    static integer i__, j;
    static doublecomplex s;
    static doublereal bscal, x, y, z__;
...
/* L90: */
	...
	if (upper) {
		...
		i__1 = *n - 3;
		i__2 = *lda + 1;
		zcopy_(&i__1, &work[1], &c__1, &a_ref(2, 3), &i__2);
		if (*n > 4) {
		    i__1 = *n - 4;
		    i__2 = *lda + 1;
		    zcopy_(&i__1, &work[*n + 1], &c__1, &a_ref(2, 4), &i__2);
		}
	    }
	    i__1 = *n - 1;
	    for (j = 2; j <= i__1; ++j) {
...
/* L100: */
		...
		i__1 = *n - 3;
		i__2 = *lda + 1;
		zcopy_(&i__1, &work[1], &c__1, &a_ref(3, 2), &i__2);
		if (*n > 4) {
		    i__1 = *n - 4;
		    i__2 = *lda + 1;
		    zcopy_(&i__1, &work[*n + 1], &c__1, &a_ref(4, 2), &i__2);
		}
	    }
	    i__1 = *n - 1;
	    for (j = 2; j <= i__1; ++j) {
...
/*              Multiply by [ c  s; -conjg(s)  c] on the left. */
		...
		if (*n > j + 1) {
		    i__2 = *n - j - 1;
		    zrot_(&i__2, &a_ref(j, j + 2), lda, &a_ref(j + 1, j + 2), 
			    lda, &c__, &s);
		}
...
/*              Multiply by [-c -s;  conjg(s) -c] on the right. */
		...
		if (j > 1) {
		    ...
		    zrot_(&i__2, &a_ref(1, j + 1), &c__1, &a_ref(1, j), &c__1,
			     &d__1, &z__1);
		}
...
/*              Multiply by [ c -s;  conjg(s) c] on the right. */
		...
		if (*n > j + 1) {
		    i__2 = *n - j - 1;
		    z__1.r = -s.r, z__1.i = -s.i;
		    zrot_(&i__2, &a_ref(j + 2, j + 1), &c__1, &a_ref(j + 2, j)
			    , &c__1, &c__, &z__1);
		}
...
/*              Multiply by [-c  s; -conjg(s) -c] on the left. */
		...
		if (j > 1) {
		    i__2 = j - 1;
		    d__1 = -c__;
		    zrot_(&i__2, &a_ref(j, 1), lda, &a_ref(j + 1, 1), lda, &
			    d__1, &s);
		}
...
/*        Type 11:  Generate a triangular matrix with elements between   
	...
	if (upper) {
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = j - 1;
		zlarnv_(&c__4, &iseed[1], &i__2, &a_ref(1, j));
		i__2 = a_subscr(j, j);
		zlarnd_(&z__2, &c__5, &iseed[1]);
...
/* L140: */
		...
		if (j < *n) {
		    i__2 = *n - j;
		    zlarnv_(&c__4, &iseed[1], &i__2, &a_ref(j + 1, j));
		}
		i__2 = a_subscr(j, j);
		zlarnd_(&z__2, &c__5, &iseed[1]);
		z__1.r = z__2.r * 2., z__1.i = z__2.i * 2.;
...
/* Computing MAX */
	...
	if (upper) {
		...
		zlarnv_(&c__4, &iseed[1], &i__2, &a_ref(1, j));
		i__2 = j - 1;
		zdscal_(&i__2, &tscal, &a_ref(1, j), &c__1);
		i__2 = a_subscr(j, j);
...
/* L160: */
		...
		if (j < *n) {
		    ...
		    zlarnv_(&c__4, &iseed[1], &i__2, &a_ref(j + 1, j));
		    i__2 = *n - j;
		    zdscal_(&i__2, &tscal, &a_ref(j + 1, j), &c__1);
		}
		i__2 = a_subscr(j, j);
		zlarnd_(&z__1, &c__5, &iseed[1]);
		a[i__2].r = z__1.r, a[i__2].i = z__1.i;
...
/* Computing MAX */
	...
	if (upper) {
	    i__1 = *n;
	    for (j = 1; j <= i__1; ++j) {
		zlarnv_(&c__5, &iseed[1], &j, &a_ref(1, j));
...
/* L390: */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j + 1;
		zlarnv_(&c__5, &iseed[1], &i__2, &a_ref(j, j));
		i__2 = *n - j + 1;
...
/*     Flip the matrix if the transpose will be used. */
	...
	if (upper) {
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - (j << 1) + 1;
		zswap_(&i__2, &a_ref(j, j), lda, &a_ref(j + 1, *n - j + 1), &
			c_n1);
/* L420: */
	    }
	} else {
		...
		i__2 = *n - (j << 1) + 1;
		i__3 = -(*lda);
		zswap_(&i__2, &a_ref(j, j), &c__1, &a_ref(*n - j + 1, j + 1), 
			&i__3);
/* L430: */
	    }
	}
