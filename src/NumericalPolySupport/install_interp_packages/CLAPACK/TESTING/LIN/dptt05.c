	...
	berr, doublereal *reslts)
{
    ...
    static integer nz;
    static doublereal errbnd, eps, tmp;
...
/*     Test 2:  Compute the maximum of BERR / ( NZ*EPS + (*) ), where   
    ...
    for (k = 1; k <= i__1; ++k) {
	if (*n == 1) {
	    axbi = (d__1 = b_ref(1, k), abs(d__1)) + (d__2 = d__[1] * x_ref(1,
		     k), abs(d__2));
	} else {
	    axbi = (d__1 = b_ref(1, k), abs(d__1)) + (d__2 = d__[1] * x_ref(1,
		     k), abs(d__2)) + (d__3 = e[1] * x_ref(2, k), abs(d__3));
	    i__2 = *n - 1;
	    for (i__ = 2; i__ <= i__2; ++i__) {
		tmp = (d__1 = b_ref(i__, k), abs(d__1)) + (d__2 = e[i__ - 1] *
			 x_ref(i__ - 1, k), abs(d__2)) + (d__3 = d__[i__] * 
			x_ref(i__, k), abs(d__3)) + (d__4 = e[i__] * x_ref(
			i__ + 1, k), abs(d__4));
		axbi = min(axbi,tmp);
/* L40: */
	    }
	    tmp = (d__1 = b_ref(*n, k), abs(d__1)) + (d__2 = e[*n - 1] * 
		    x_ref(*n - 1, k), abs(d__2)) + (d__3 = d__[*n] * x_ref(*n,
		     k), abs(d__3));
	    axbi = min(axbi,tmp);
	}
/* Computing MAX */
