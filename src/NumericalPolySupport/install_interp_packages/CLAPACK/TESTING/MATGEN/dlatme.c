...
/*     3)      If UPPER='T', set upper triangle of A to random numbers.   
	...
	for (jc = 2; jc <= i__1; ++jc) {
	    ...
	    dlarnv_(&idist, &iseed[1], &jr, &a_ref(1, jc));
/* L70: */
	}
    }
...
/*        Multiply by S and (1/S) */
	...
	i__1 = *n;
	for (j = 1; j <= i__1; ++j) {
	    dscal_(n, &ds[j], &a_ref(j, 1), lda);
	    if (ds[j] != 0.) {
		d__1 = 1. / ds[j];
		dscal_(n, &d__1, &a_ref(1, j), &c__1);
	    } else {
		*info = 5;
		return 0;
	    }
...
/*        Reduce bandwidth -- kill column */
	...
	for (jcr = *kl + 1; jcr <= i__1; ++jcr) {
	    ...
	    a_ref(jcr, ic) = xnorms;
	    i__2 = irows - 1;
	    dlaset_("Full", &i__2, &c__1, &c_b23, &c_b23, &a_ref(jcr + 1, ic),
		     lda);
/* L90: */
	}
    } else if (*ku < *n - 1) {
...
/*        Reduce upper bandwidth -- kill a row at a time. */
	...
	for (jcr = *ku + 1; jcr <= i__1; ++jcr) {
	    ...
	    a_ref(ir, jcr) = xnorms;
	    i__2 = icols - 1;
	    dlaset_("Full", &c__1, &i__2, &c_b23, &c_b23, &a_ref(ir, jcr + 1),
		     lda);
/* L100: */
	}
    }
