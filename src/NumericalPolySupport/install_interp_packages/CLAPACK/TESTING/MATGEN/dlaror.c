...
/*           Apply H(k) from the left. */
	    ...
	    dgemv_("T", &ixfrm, n, &c_b10, &a_ref(kbeg, 1), lda, &x[kbeg], &
...
/*           Apply H(k) from the right. */
	    ...
	    dgemv_("N", m, &ixfrm, &c_b10, &a_ref(1, kbeg), lda, &x[kbeg], &
		    c__1, &c_b9, &x[(nxfrm << 1) + 1], &c__1);
	    d__1 = -factor;
...
/*     Scale the matrix A by D. */
	...
	for (irow = 1; irow <= i__1; ++irow) {
	    dscal_(n, &x[nxfrm + irow], &a_ref(irow, 1), lda);
/* L40: */
	}
    }
	...
	for (jcol = 1; jcol <= i__1; ++jcol) {
	    dscal_(m, &x[nxfrm + jcol], &a_ref(1, jcol), &c__1);
/* L50: */
	}
    }
    return 0;
