...
/*           Apply H(k) on the left of A */
	    ...
	    zgemv_("C", &ixfrm, n, &c_b2, &a_ref(kbeg, 1), lda, &x[kbeg], &
...
/*           Apply H(k)* (or H(k)') on the right of A */
	    ...
	    zgemv_("N", m, &ixfrm, &c_b2, &a_ref(1, kbeg), lda, &x[kbeg], &
		    c__1, &c_b1, &x[(nxfrm << 1) + 1], &c__1);
	    z__2.r = factor, z__2.i = 0.;
	    z__1.r = -z__2.r, z__1.i = -z__2.i;
...
/*     Scale the matrix A by D. */
	...
	for (irow = 1; irow <= i__1; ++irow) {
	    ...
	    zscal_(n, &z__1, &a_ref(irow, 1), lda);
/* L40: */
	}
    }
	...
	for (jcol = 1; jcol <= i__1; ++jcol) {
	    zscal_(m, &x[nxfrm + jcol], &a_ref(1, jcol), &c__1);
/* L50: */
	}
    }
	...
	for (jcol = 1; jcol <= i__1; ++jcol) {
	    ...
	    zscal_(m, &z__1, &a_ref(1, jcol), &c__1);
/* L60: */
	}
    }
    return 0;
