...
/*     Diagonal Matrix -- We are done, unless it   
    ...
    if (llb == 0 && uub == 0) {
	i__1 = ilda + 1;
	scopy_(&mnmin, &d__[1], &c__1, &a_ref(1 - iskew + ioffst, 1), &i__1);
	if (ipack <= 2 || ipack >= 5) {
	    ipackg = ipack;
	}
...
/*           Non-symmetric -- A = U D V */
	    ...
	    i__1 = ilda + 1;
	    scopy_(&mnmin, &d__[1], &c__1, &a_ref(1 - iskew + ioffst, 1), &
		    i__1);
...
/*                    Chase "EXTRA" back up */
			...
			for (jch = jr - jkl; i__3 < 0 ? jch >= 1 : jch <= 1; 
				jch += i__3) {
			    if (ir < *m) {
				slartg_(&a_ref(ir + 1 - iskew * (ic + 1) + 
					ioffst, ic + 1), &extra, &c__, &s, &
					dummy);
			    }
/* Computing MAX */
			    i__4 = 1, i__5 = jch - jku;
			    ...
			    slarot_(&c_false, &iltemp, &c_true, &il, &c__, &
				    r__1, &a_ref(irow - iskew * ic + ioffst, 
				    ic), &ilda, &temp, &extra);
			    if (iltemp) {
				slartg_(&a_ref(irow + 1 - iskew * (ic + 1) + 
					ioffst, ic + 1), &temp, &c__, &s, &
					dummy);
/* Computing MAX */
				...
				slarot_(&c_true, &L__1, &c_true, &il, &c__, &
					r__1, &a_ref(irow - iskew * icol + 
					ioffst, icol), &ilda, &extra, &temp);
				ic = icol;
				ir = irow;
			    }
/* L30: */
...
/* Computing MAX */
			    ...
			    slarot_(&c_true, &iltemp, &c_true, &il, &c__, &
				    r__1, &a_ref(ir - iskew * icol + ioffst, 
				    icol), &ilda, &temp, &extra);
			    if (iltemp) {
				slartg_(&a_ref(ir + 1 - iskew * (icol + 1) + 
					ioffst, icol + 1), &temp, &c__, &s, &
					dummy);
/* Computing MAX */
...
/*                    Chase "EXTRA" back down */
			...
			for (jch = jc + jkl; i__4 < 0 ? jch >= i__2 : jch <= 
			    ...
			    ilextr = ic > 0;
			    if (ilextr) {
				slartg_(&a_ref(jch - iskew * ic + ioffst, ic),
					 &extra, &c__, &s, &dummy);
			    }
			    ic = max(1,ic);
/* Computing MIN */
			    ...
			    slarot_(&c_true, &ilextr, &iltemp, &i__5, &c__, &
				    ...
				    &ilda, &extra, &temp);
			    if (iltemp) {
				slartg_(&a_ref(jch - iskew * icol + ioffst, 
					icol), &temp, &c__, &s, &dummy);
/* Computing MIN */
...
/*                    Chase "EXTRA" back down */
			...
			for (jch = jr + jku; i__2 < 0 ? jch >= i__3 : jch <= 
			    ...
			    ilextr = ir > 0;
			    if (ilextr) {
				slartg_(&a_ref(ir - iskew * jch + ioffst, jch)
					, &extra, &c__, &s, &dummy);
			    }
			    ir = max(1,ir);
/* Computing MIN */
			    ...
			    slarot_(&c_false, &ilextr, &iltemp, &i__5, &c__, &
				    s, &a_ref(ir - iskew * jch + ioffst, jch),
				     &ilda, &extra, &temp);
			    if (iltemp) {
				slartg_(&a_ref(irow - iskew * jch + ioffst, 
					jch), &temp, &c__, &s, &dummy);
/* Computing MIN */
				i__5 = iendch, i__6 = jch + jkl + jku;
				il = min(i__5,i__6) + 2 - jch;
				extra = 0.f;
...
/*              Top-Down -- Generate Upper triangle only */
		...
		i__1 = ilda + 1;
		scopy_(&mnmin, &d__[1], &c__1, &a_ref(1 - iskew + ioffg, 1), &
			i__1);
...
/* Computing MIN */
			...
			il = min(i__2,i__3);
			extra = 0.f;
			temp = a_ref(jc - iskew * (jc + 1) + ioffg, jc + 1);
			angle = slarnd_(&c__1, &iseed[1]) * 
				6.2831853071795864769252867663f;
			c__ = cos(angle);
			s = sin(angle);
			L__1 = jc > k;
			slarot_(&c_false, &L__1, &c_true, &il, &c__, &s, &
...
/*                    Chase EXTRA back up the matrix */
			...
			for (jch = jc - k; i__2 < 0 ? jch >= 1 : jch <= 1; 
				jch += i__2) {
			    slartg_(&a_ref(jch + 1 - iskew * (icol + 1) + 
				    ioffg, icol + 1), &extra, &c__, &s, &
				    dummy);
			    temp = a_ref(jch - iskew * (jch + 1) + ioffg, jch 
				    + 1);
			    i__3 = k + 2;
			    r__1 = -s;
			    slarot_(&c_true, &c_true, &c_true, &i__3, &c__, &
				    r__1, &a_ref((1 - iskew) * jch + ioffg, 
				    jch), &ilda, &temp, &extra);
/* Computing MAX */
			    i__3 = 1, i__5 = jch - k;
			    irow = max(i__3,i__5);
/* Computing MIN */
			    ...
			    r__1 = -s;
			    slarot_(&c_false, &L__1, &c_true, &il, &c__, &
				    r__1, &a_ref(irow - iskew * jch + ioffg, 
				    jch), &ilda, &extra, &temp);
			    icol = jch;
/* L150: */
			}
...
/* Computing MIN */
			...
			for (jr = jc; jr <= i__4; ++jr) {
			    a_ref(jr + irow, jc) = a_ref(jc - iskew * jr + 
				    ioffg, jr);
/* L180: */
			}
/* L190: */
...
/* Computing MIN */
			...
			il = min(i__4,i__2);
			extra = 0.f;
			temp = a_ref((1 - iskew) * jc + 1 + ioffg, jc);
			angle = slarnd_(&c__1, &iseed[1]) * 
				6.2831853071795864769252867663f;
			c__ = cos(angle);
			s = -sin(angle);
...
/*                    Chase EXTRA back down the matrix */
			...
			for (jch = jc + k; i__2 < 0 ? jch >= i__4 : jch <= 
				i__4; jch += i__2) {
			    slartg_(&a_ref(jch - iskew * icol + ioffg, icol), 
				    &extra, &c__, &s, &dummy);
			    temp = a_ref((1 - iskew) * jch + 1 + ioffg, jch);
			    i__3 = k + 2;
			    slarot_(&c_true, &c_true, &c_true, &i__3, &c__, &
				    s, &a_ref(jch - iskew * icol + ioffg, 
				    icol), &ilda, &extra, &temp);
/* Computing MIN */
...
/* Computing MIN */
		...
		for (i__ = min(i__2,*m); i__ >= 1; --i__) {
		    a_ref(i__ - j + uub + 1, j) = a_ref(i__, j);
/* L370: */
		}
/* L380: */
	    }
