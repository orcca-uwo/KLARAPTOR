...
/*        Use SLATM2 */
	...
	if (ipack == 0) {
		...
		for (j = 1; j <= i__1; ++j) {
		    i__2 = j;
		    for (i__ = 1; i__ <= i__2; ++i__) {
			a_ref(i__, j) = slatm2_(m, n, &i__, &j, kl, ku, &
				idist, &iseed[1], &d__[1], &igrade, &dl[1], &
				dr[1], &ipvtng, &iwork[1], sparse);
...
/* L320: */
		...
		i__2 = j;
		for (i__ = 1; i__ <= i__2; ++i__) {
		    a_ref(i__, j) = slatm2_(m, n, &i__, &j, kl, ku, &idist, &
...
/* L340: */
		...
		i__2 = j;
		for (i__ = 1; i__ <= i__2; ++i__) {
		    a_ref(j, i__) = slatm2_(m, n, &i__, &j, kl, ku, &idist, &
			    iseed[1], &d__[1], &igrade, &dl[1], &dr[1], &
			    ipvtng, &iwork[1], sparse);
		    if (i__ != j) {
			a_ref(i__, j) = 0.f;
...
/* L360: */
		...
		for (i__ = 1; i__ <= i__2; ++i__) {
		    ...
		    a_ref(isub, jsub) = slatm2_(m, n, &i__, &j, kl, ku, &
			    idist, &iseed[1], &d__[1], &igrade, &dl[1], &dr[1]
			    , &ipvtng, &iwork[1], sparse);
/* L370: */
		}
...
/* L420: */
		...
		for (i__ = j - kuu; i__ <= i__2; ++i__) {
			...
			a_ref(j - i__ + 1, i__) = slatm2_(m, n, &i__, &j, kl, 
...
/* L440: */
		...
		i__2 = j;
		for (i__ = j - kuu; i__ <= i__2; ++i__) {
		    a_ref(i__ - j + kuu + 1, j) = slatm2_(m, n, &i__, &j, kl, 
...
/* L460: */
		...
		for (j = 1; j <= i__1; ++j) {
		    i__2 = j;
		    for (i__ = j - kuu; i__ <= i__2; ++i__) {
			a_ref(i__ - j + kuu + 1, j) = slatm2_(m, n, &i__, &j, 
...
/*           Scale carefully to avoid over / underflow */
		...
		for (j = 1; j <= i__1; ++j) {
		    r__1 = 1.f / onorm;
		    sscal_(m, &r__1, &a_ref(1, j), &c__1);
		    sscal_(m, anorm, &a_ref(1, j), &c__1);
/* L510: */
		}
		...
		for (j = 1; j <= i__1; ++j) {
		    i__2 = kll + kuu + 1;
		    r__1 = 1.f / onorm;
		    sscal_(&i__2, &r__1, &a_ref(1, j), &c__1);
		    i__2 = kll + kuu + 1;
		    sscal_(&i__2, anorm, &a_ref(1, j), &c__1);
/* L520: */
		}
