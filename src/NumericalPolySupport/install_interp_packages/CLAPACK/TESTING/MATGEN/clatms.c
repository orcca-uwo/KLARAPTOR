...
/* Computing MAX */
			    ...
			    clarot_(&c_false, &iltemp, &c_true, &il, &c__, &s,
				    ...
				    ilda, &ctemp, &extra);
			    if (iltemp) {
				clartg_(&a_ref(irow + 1 - iskew * (ic + 1) + 
...
/* Computing MAX */
			    ...
			    clarot_(&c_true, &iltemp, &c_true, &il, &c__, &s, 
				    ...
				    &ilda, &ctemp, &extra);
			    if (iltemp) {
				clartg_(&a_ref(ir + 1 - iskew * (icol + 1) + 
...
/*                    Chase "EXTRA" back down */
			...
			for (jch = jc + jkl; i__4 < 0 ? jch >= i__2 : jch <= 
			    ...
			    ilextr = ic > 0;
			    if (ilextr) {
				clartg_(&a_ref(jch - iskew * ic + ioffst, ic),
					 &extra, &realc, &s, &dummy);
				clarnd_(&q__1, &c__5, &iseed[1]);
				dummy.r = q__1.r, dummy.i = q__1.i;
...
/* Computing MIN */
			    ...
			    clarot_(&c_true, &ilextr, &iltemp, &i__5, &c__, &
				    ...
				    &ilda, &extra, &ctemp);
			    if (iltemp) {
				clartg_(&a_ref(jch - iskew * icol + ioffst, 
...
/*                    Chase "EXTRA" back down */
			...
			for (jch = jr + jku; i__2 < 0 ? jch >= i__3 : jch <= 
			    ...
			    ilextr = ir > 0;
			    if (ilextr) {
				clartg_(&a_ref(ir - iskew * jch + ioffst, jch)
					, &extra, &realc, &s, &dummy);
				clarnd_(&q__1, &c__5, &iseed[1]);
				dummy.r = q__1.r, dummy.i = q__1.i;
				q__1.r = realc * dummy.r, q__1.i = realc * 
...
/* Computing MIN */
			    ...
			    clarot_(&c_false, &ilextr, &iltemp, &i__5, &c__, &
				    s, &a_ref(ir - iskew * jch + ioffst, jch),
				     &ilda, &extra, &ctemp);
			    if (iltemp) {
				clartg_(&a_ref(irow - iskew * jch + ioffst, 
...
/* Computing MIN */
			...
			L__1 = jc > k;
			clarot_(&c_false, &L__1, &c_true, &il, &c__, &s, &
...
/*                    Chase EXTRA back up the matrix */
			...
			for (jch = jc - k; i__2 < 0 ? jch >= 1 : jch <= 1; 
				jch += i__2) {
			    clartg_(&a_ref(jch + 1 - iskew * (icol + 1) + 
...
/* Computing MIN */
			    ...
			    for (jr = jc; jr <= i__4; ++jr) {
				i__2 = a_subscr(jr + irow, jc);
				r_cnjg(&q__1, &a_ref(jc - iskew * jr + ioffg, 
					jr));
				a[i__2].r = q__1.r, a[i__2].i = q__1.i;
/* L220: */
			    }
...
/* Computing MIN */
			...
			L__1 = *n - jc > k;
			clarot_(&c_false, &c_true, &L__1, &il, &c__, &s, &
				a_ref((1 - iskew) * jc + ioffg, jc), &ilda, &
				ctemp, &extra);
/* Computing MAX */
			i__4 = 1, i__2 = jc - k + 1;
...
/*                    Chase EXTRA back down the matrix */
			...
			for (jch = jc + k; i__2 < 0 ? jch >= i__4 : jch <= 
				i__4; jch += i__2) {
			    clartg_(&a_ref(jch - iskew * icol + ioffg, icol), 
				    &extra, &realc, &s, &dummy);
			    clarnd_(&q__1, &c__5, &iseed[1]);
			    dummy.r = q__1.r, dummy.i = q__1.i;
			    q__1.r = realc * dummy.r, q__1.i = realc * 
