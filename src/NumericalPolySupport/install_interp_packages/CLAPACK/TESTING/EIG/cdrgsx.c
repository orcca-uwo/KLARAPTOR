...
/* Computing MAX */
			...
			r__13 = smlnum, r__14 = (r__1 = alpha[i__8].r, dabs(
				...
				ai[i__9].r, dabs(r__3)) + (r__4 = r_imag(&
				ai_ref(j, j)), dabs(r__4));
/* Computing MAX */
			i__10 = j;
			i__11 = bi_subscr(j, j);
			r__15 = smlnum, r__16 = (r__5 = beta[i__10].r, dabs(
				...
				bi[i__11].r, dabs(r__7)) + (r__8 = r_imag(&
				bi_ref(j, j)), dabs(r__8));
...
/*                    Note: for either following two cases, there are   
			...
			i__3 = mn_1.mplusn - mm;
			clakf2_(&mm, &i__3, &ai[ai_offset], lda, &ai_ref(mm + 
...
L80:
	...
	i__2 = mn_1.mplusn;
	for (j = 1; j <= i__2; ++j) {
	    do_lio(&c__6, &c__1, (char *)&ai_ref(i__, j), (ftnlen)sizeof(
		    complex));
	}
	e_rsle();
/* L90: */
	...
	i__2 = mn_1.mplusn;
	for (j = 1; j <= i__2; ++j) {
	    do_lio(&c__6, &c__1, (char *)&bi_ref(i__, j), (ftnlen)sizeof(
		    complex));
	}
	e_rsle();
/* L100: */
...
/* Computing MAX */
	...
	r__13 = smlnum, r__14 = (r__1 = alpha[i__6].r, dabs(r__1)) + (r__2 = 
		...
		r__14 = (r__3 = ai[i__7].r, dabs(r__3)) + (r__4 = r_imag(&
		ai_ref(j, j)), dabs(r__4));
/* Computing MAX */
	i__8 = j;
	i__9 = bi_subscr(j, j);
	r__15 = smlnum, r__16 = (r__5 = beta[i__8].r, dabs(r__5)) + (r__6 = 
		...
		r__16 = (r__7 = bi[i__9].r, dabs(r__7)) + (r__8 = r_imag(&
		bi_ref(j, j)), dabs(r__8));
