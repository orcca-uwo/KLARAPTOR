...
/*        ITYPE=1: error = A - U S U' */
	...
	for (j = 1; j <= i__1; ++j) {
	    r__1 = -d__[j];
	    ssyr_(cuplo, n, &r__1, &u_ref(1, j), &c__1, &work[1], n);
/* L10: */
	}
	...
	if (*n > 1 && *kband == 1) {
	    ...
	    for (j = 1; j <= i__1; ++j) {
		r__1 = -e[j];
		ssyr2_(cuplo, n, &r__1, &u_ref(1, j), &c__1, &u_ref(1, j + 1),
			 &c__1, &work[1], n);
/* L20: */
	    }
	}
...
/* Computing 2nd power */
		...
		if (*kband == 1) {
			...
			work[(j - 1) * *n + jr] = -tau[j] * e[j] * v_ref(jr, 
				j);
/* L30: */
		    }
		}
...
/* Computing 2nd power */
		i__2 = *n;
		slarfy_("L", &i__1, &v_ref(j + 1, j), &c__1, &tau[j], &work[(*
			n + 1) * j + 1], n, &work[i__2 * i__2 + 1])
			;
		v_ref(j + 1, j) = vsave;
...
/* L40: */
		...
		if (*kband == 1) {
		    ...
		    i__2 = j - 1;
		    for (jr = 1; jr <= i__2; ++jr) {
			work[j * *n + jr] = -tau[j] * e[j] * v_ref(jr, j + 1);
/* L50: */
		    }
		}
...
/* Computing 2nd power */
		i__2 = *n;
		slarfy_("U", &j, &v_ref(1, j + 1), &c__1, &tau[j], &work[1], 
			n, &work[i__2 * i__2 + 1]);
		v_ref(j, j + 1) = vsave;
...
/* Computing 2nd power */
	    i__3 = *n;
	    sorm2r_("R", "T", n, &i__1, &i__2, &v_ref(2, 1), ldv, &tau[1], &
...
/* Computing 2nd power */
	    i__3 = *n;
	    sorm2l_("R", "T", n, &i__1, &i__2, &v_ref(1, 2), ldv, &tau[1], &
		    work[1], n, &work[i__3 * i__3 + 1], &iinfo);
	}
	if (iinfo != 0) {
	    result[1] = 10.f / ulp;
	    return 0;
