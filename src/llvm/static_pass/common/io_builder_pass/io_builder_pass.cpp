...
bool func_insert::create_maps_in_connector(Module & M)
{
	...
	FunctionCallee init_kernel_info_dict_func = F.getParent()->getOrInsertFunction(
	...
	FunctionCallee add_to_kernel_info_dict_func = F.getParent()->getOrInsertFunction(
	...
	FunctionCallee init_cu_driver_call_func = F.getParent()->getOrInsertFunction(
...
//	void get_best_config_ptr_all_info(char *kernel_name, void* kernel_params, int *launch_params)
	...
	FunctionCallee get_best_config_func = F.getParent()->getOrInsertFunction(
...
//			errs()<<"a new one++;\n";
			...
			{
				if (auto x1 = dyn_cast<ConstantData>(x0->getInitializer()))
				{
