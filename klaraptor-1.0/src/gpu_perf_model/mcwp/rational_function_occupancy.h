/*!
 \file rational_function_occupancy.h
...
void print_to_header(FILE* occupancy_header_file,
		poly_params kernel_poly_params, int k) {
	if (k == 0) {
...
int test_mwp_cwp(int argc, char **argv, int n_kernels, int nvar,
		int* degree_bounds, int* denomdegree_bounds, char* mem_inst_trace_tmp,
		enum kernel_type_enum kernel_type, int (*rep_div_n)(int)) {
...
//						N_PROGRAM_PARAMS, global_program_params_list);
				if (read_params_from_file(avg_file_path_list[j], program_params,
						N_PROGRAM_PARAMS,
						global_program_params_list) != EXIT_SUCCESS) {
...
//				fprintf(stderr, "For Actual blocks per SM:\n");
				print_to_header(occupancy_header_file, kernel_poly_params[j],
						k);
