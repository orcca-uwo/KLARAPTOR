...
char* polyToProgram_AA(AltArr_t* aa, char* polyVar) {
	...
	size_t gmpPrintedSize;
	size_t mpzStrSize;
	size_t polyVarSize = strlen(polyVar);
	size_t mpzConstSize = 50 + 10; //10 is max size of int in decimal for elems index
	for (int i = 0; i < aa->size; ++i) {
		lineLength = 0;
		...
		//num coef
		lineLength = 0;
		mpzStrSize = mpz_sizeinbase(mpq_numref(aa->elems[i].coef), 10) + 2;	
		if (mpzStrSize + polyVarSize + mpzConstSize >= lineAlloc - lineLength) {
			while (mpzStrSize + polyVarSize + mpzConstSize >= lineAlloc - lineLength) {
				lineAlloc <<= 1;
			}
		...
		while (gmpPrintedSize + strLength + 1 > strAlloc) {
			strAlloc <<= 1;
			str = (char*) realloc(str, strAlloc*sizeof(char));
		}
		strLength += sprintf(str + strLength, "%s", line);
		...
		//den coef
		lineLength = 0;
		mpzStrSize = mpz_sizeinbase(mpq_denref(aa->elems[i].coef), 10) + 2;     
                if (mpzStrSize + polyVarSize + mpzConstSize >= lineAlloc - lineLength) {
                        while (mpzStrSize + polyVarSize + mpzConstSize >= lineAlloc - lineLength) {
                                lineAlloc <<= 1;
                        }    
