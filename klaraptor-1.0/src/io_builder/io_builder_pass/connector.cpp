...
//  kernel_history_t* kernel_history_list;
} kernel_info_dictionary_default = { NOT_INITIALIZED };
...
///////////////////////////////////////
static kernel_info_dictionary_t global_kernel_dim_map = { NOT_INITIALIZED,
		"global_kernel_dim_map", "kernel_info_dim" };
///////////////////////////////////////
...
void init_kernel_info_dict(char * dict_name, int size) {
#if VERBOSE
	printf("[@%s]"
...
void add_to_kernel_info_dict(char* dict_name, char*kernel_param_str,
		int value) {
	int verbose = 0;
	kernel_info_dictionary_t * ptr = get_ptr_to_kernel_info_dict(dict_name);
...
int get_value_from_kernel_info_dict(char *dict_name, char * kernel_name,
		int *size_idx) {
...
int strip_kernel_name(char *kernel_name) {
...
void get_best_config(const char *kernel_name_in, int *launch_params,
	...
	//TODO: rewrite "strip_kernel_name"
	...
	if (get_value_from_kernel_info_dict("global_kernel_size_param_idx_map",
			kernel_name, &kernel_size_idx) != EXIT_SUCCESS) {
...
//    int kernel_idx=-1;
//    int stat=kernel_info_dict_get_idx(&kernel_idx)
//    int bx=0, by=0, bz=0;
//   if (kernel_history_get_config(&bx, &by, &bz, ))
...
void kernel_info_dictionary_clear(void) __attribute__ ((destructor))
{
//  printf ("this is JUST CLEARING KERNEL INFO DICT\n");
}
