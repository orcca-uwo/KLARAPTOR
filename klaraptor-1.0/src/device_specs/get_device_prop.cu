/* This code is written on top of device query sample from CUDA SDK. 
 * Please see: 
 * 		https://docs.nvidia.com/cuda/cuda-samples/index.html#device-query
 */
...
#define DEFAULT_DEPARTURE_DELAY 1
...
get_nblocks_per_SM (int major, int minor)
{
  ...
  sSMtoCores nGpuArchCoresPerSM[] =
    {
      { 0x20, 8 }, // Fermi Generation (SM 2.0) GF100 class
	  { 0x21, 8 }, // Fermi Generation (SM 2.1) GF10x class
...
//	unsigned long long int peak_bw_kflops = 2 * (memory_bus_width / 8) * memory_clock_khz;
  unsigned long long int peak_bw_kflops = 2*(memory_bus_width/8)
      * memory_clock_khz;
...
//      fprintf (file, "[Departure_del_uncoal: %d]\n", DEFAULT_DEPARTURE_DELAY);
      ...
      fprintf(file, "[Compute_Capability: %d%d]\n",(int)deviceProp.major, (int)deviceProp.minor);
